...
private:
	using HashTable = hamon::detail::hash_table<key_type, value_type, hasher, key_equal, allocator_type, false>;
...
public:
	...
	unordered_set()	// may throw
		: unordered_set(size_type())	// [unord.set.cnstr]/1
	{
	...
	unordered_set(
		...
		: m_allocator(a)
		, m_impl(n, hf, eql, a)	// [unord.set.cnstr]/1
	{}

	HAMON_CXX14_CONSTEXPR
	unordered_set(size_type n, hasher const& hf, allocator_type const& a)	// may throw
	...
	HAMON_CXX14_CONSTEXPR
	unordered_set(size_type n, allocator_type const& a)	// may throw
	...
	unordered_set(
		...
		: unordered_set(f, l, n, hf, key_equal(), a)	// may throw
	{
	...
	template <HAMON_CONSTRAINT(hamon::detail::cpp17_input_iterator, InputIterator)>
	HAMON_CXX14_CONSTEXPR
	unordered_set(InputIterator f, InputIterator l, allocator_type const& a)	// may throw
		: unordered_set(f, l, size_type(), hasher(), key_equal(), a)
	{
	...
	template <HAMON_CONSTRAINT(hamon::detail::container_compatible_range, value_type, R)>
	HAMON_CXX14_CONSTEXPR
	unordered_set(hamon::from_range_t, R&& rg, allocator_type const& a)	// may throw
		: unordered_set(hamon::from_range, hamon::forward<R>(rg), size_type(), hasher(), key_equal(), a)
	{
	...
	HAMON_CXX14_CONSTEXPR explicit
	unordered_set(Allocator const& a)	// may throw
		: unordered_set(size_type(), hasher(), key_equal(), a)
	{}

	HAMON_CXX14_CONSTEXPR
	unordered_set(unordered_set const& x)	// may throw
		: unordered_set(x.bucket_count(), x.hash_function(), x.key_eq(),
	...
	unordered_set(unordered_set&& x) HAMON_NOEXCEPT_IF(	// noexcept as an extension
	...
	HAMON_CXX14_CONSTEXPR
	unordered_set(unordered_set&& x, hamon::type_identity_t<Allocator> const& a)
	...
	HAMON_CXX14_CONSTEXPR unordered_set&
	operator=(unordered_set&& x) HAMON_NOEXCEPT_IF(
		AllocTraits::is_always_equal::value &&
		hamon::is_nothrow_move_assignable<Hash>::value &&
		hamon::is_nothrow_move_assignable<Pred>::value)
	{
	...
	template <typename K,
		...
		HAMON_CONSTRAINT_D(hamon::detail::has_is_transparent, H, Hash),
		HAMON_CONSTRAINT_D(hamon::detail::has_is_transparent, P, Pred)>
	HAMON_CXX14_CONSTEXPR hamon::pair<iterator, bool>
	insert(K&& obj)
	{
		...
		return this->emplace(hamon::forward<K>(obj));	// may throw
	}
...
//		requires (!hamon::same_as<iterator, const_iterator>)
	...
	HAMON_CXX14_CONSTEXPR void
	swap(unordered_set& x) HAMON_NOEXCEPT_IF(
		AllocTraits::is_always_equal::value &&
...
template <
	...
	HAMON_CONSTRAINT_D(hamon::detail::simple_allocator, Allocator,
...
template <
	...
	HAMON_CONSTRAINED_PARAM_D(hamon::detail::is_hasher, Hash, hamon::hash<ranges::range_value_t<R>>),
	HAMON_CONSTRAINT_D(hamon::detail::not_simple_allocator, Pred, hamon::equal_to<ranges::range_value_t<R>>),
	HAMON_CONSTRAINT_D(hamon::detail::simple_allocator, Allocator, hamon::allocator<ranges::range_value_t<R>>)>
unordered_set(
