/**
 *	@file	unit_test_unordered_multiset_insert_hint_copy.cpp
 *
 *	@brief	insert のテスト
 *
...
UNORDERED_MULTISET_TEST_CONSTEXPR bool test1()
{
	...
	for (int i = 0; i < 100; ++i)
	{
		ValueType const x{static_cast<Key>(i)};
		auto r = v.insert(v.end(), x);
...
GTEST_TEST(UnorderedMultisetTest, InsertHintCopyTest)
{
	...
	{
		using Set = hamon::unordered_multiset<ThrowOnCopy, hamon::hash<ThrowOnCopy>>;
