 ...
 *	@file	stop_state.hpp
 *
 *	@brief	stop_state の定義
 */

#ifndef HAMON_STOP_TOKEN_DETAIL_STOP_STATE_HPP
#define HAMON_STOP_TOKEN_DETAIL_STOP_STATE_HPP

#include <hamon/stop_token/detail/atomic_unique_lock.hpp>
...
class stop_state
{
	static constexpr hamon::uint32_t __stop_requested_bit = 1;
	static constexpr hamon::uint32_t __callback_list_locked_bit = 1 << 1;
	static constexpr hamon::uint32_t __stop_source_counter_shift = 2;
	...
	std::atomic<hamon::uint32_t> __state_ = 0;
	...
	std::atomic<hamon::uint32_t> __ref_count_ = 0;

	using __state_t = hamon::uint32_t;
	using __callback_list_lock = hamon::detail::atomic_unique_lock<__state_t, __callback_list_locked_bit>;
	using __callback_list = hamon::list<stop_callback_base*>;	//__intrusive_list_view<stop_callback_base>;
...
public:
	...
	void __increment_stop_source_counter() noexcept
	{
	...
	void __decrement_stop_source_counter() noexcept
	{
		//_LIBCPP_ASSERT_UNCATEGORIZED(
		//	__state_.load(std::memory_order_relaxed) >= static_cast<__state_t>(1 << __stop_source_counter_shift),
	...
	bool __stop_requested() const noexcept
	{
	...
	bool __stop_possible_for_stop_token() const noexcept
	{
	...
	bool __request_stop() noexcept
	{
		...
		while (!__callback_list_.empty())
		{
			auto __cb = __callback_list_.front();
			__callback_list_.pop_front();
	...
	bool __add_callback(stop_callback_base* __cb) noexcept
	{
		...
		__callback_list_.push_front(__cb);
	...
	void __remove_callback(stop_callback_base* __cb) noexcept
	{
		...
		bool __potentially_executing_now = true;//__cb->__prev_ == nullptr && !__callback_list_.__is_head(__cb);
...
#endif // HAMON_STOP_TOKEN_DETAIL_STOP_STATE_HPP
