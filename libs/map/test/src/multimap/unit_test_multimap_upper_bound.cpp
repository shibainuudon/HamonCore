/**
 *	@file	unit_test_multimap_upper_bound.cpp
 *
...
MULTIMAP_TEST_CONSTEXPR bool test1()
{
	...
	static_assert(hamon::is_same<decltype(hamon::declval<Map&>().upper_bound(hamon::declval<Key const&>())), Iterator>::value, "");
	...
	VERIFY(v.upper_bound(Key{0}) == hamon::next(v.begin(), 2));
	VERIFY(v.upper_bound(Key{1}) == hamon::next(v.begin(), 4));
	VERIFY(v.upper_bound(Key{2}) == v.end());
	VERIFY(v.upper_bound(Key{3}) == v.end());
...
MULTIMAP_TEST_CONSTEXPR bool test2()
{
	...
	static_assert(hamon::is_same<decltype(hamon::declval<Map const&>().upper_bound(hamon::declval<Key const&>())), ConstIterator>::value, "");
	...
	VERIFY(v.upper_bound(Key{0}) == hamon::next(v.begin(), 0));
	VERIFY(v.upper_bound(Key{1}) == hamon::next(v.begin(), 0));
	VERIFY(v.upper_bound(Key{2}) == hamon::next(v.begin(), 2));
	VERIFY(v.upper_bound(Key{3}) == hamon::next(v.begin(), 5));
	VERIFY(v.upper_bound(Key{4}) == hamon::next(v.begin(), 5));
	VERIFY(v.upper_bound(Key{5}) == hamon::next(v.begin(), 6));
	VERIFY(v.upper_bound(Key{6}) == hamon::next(v.begin(), 6));
	VERIFY(v.upper_bound(Key{7}) == v.end());
	VERIFY(v.upper_bound(Key{8}) == v.end());
...
GTEST_TEST(MultimapTest, UpperBoundTest)
{
