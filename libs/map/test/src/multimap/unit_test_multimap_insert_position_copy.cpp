/**
 *	@file	unit_test_multimap_insert_position_copy.cpp
 *
 *	@brief	insert のテスト
 *
...
MULTIMAP_TEST_CONSTEXPR bool test1()
{
	...
	static_assert(hamon::is_same<decltype(v.insert(hamon::declval<ConstIterator>(), hamon::declval<ValueType const&>())), Iterator>::value, "");
	static_assert(!noexcept(v.insert(hamon::declval<ConstIterator>(), hamon::declval<ValueType const&>())), "");
	...
	}
	{
		...
		auto r = v.insert(hamon::next(v.begin(), 2), x);
	...
	}
	{
		...
		auto r = v.insert(hamon::next(v.begin(), 3), x);
		VERIFY(r == hamon::next(v.begin(), 3));
	...
	{
		ValueType const x{Key{2}, T{70}};
		auto r = v.insert(hamon::next(v.begin(), 4), x);
...
GTEST_TEST(MultimapTest, InsertPositionCopyTest)
{
...
#if !defined(HAMON_NO_EXCEPTIONS)
	{
		...
		namespace ns = hamon;
#endif
		using Map = hamon::multimap<int, ThrowOnCopy>;
		using ValueType = typename Map::value_type;
