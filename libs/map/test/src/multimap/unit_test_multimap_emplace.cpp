/**
 *	@file	unit_test_multimap_emplace.cpp
 *
 *	@brief	emplace のテスト
 *
...
HAMON_CXX20_CONSTEXPR bool test1()
{
	...
	static_assert(hamon::is_same<decltype(v.emplace(hamon::declval<ValueType&&>())), Iterator>::value, "");
	static_assert(hamon::is_same<decltype(v.emplace(hamon::declval<ValueType const&>())), Iterator>::value, "");
	static_assert(hamon::is_same<decltype(v.emplace(hamon::declval<Key>(), hamon::declval<T>())), Iterator>::value, "");
...
GTEST_TEST(MultimapTest, EmplaceTest)
{
	...
	// https://en.cppreference.com/w/cpp/container/multimap/emplace
	{
		hamon::multimap<std::string, std::string> m;
		...
		m.emplace(hamon::piecewise_construct,
				  hamon::forward_as_tuple("c"),
				  hamon::forward_as_tuple(10u, 'c'));

		EXPECT_EQ("[a] = a, [b] = abcd, [c] = cccccccccc, [d] = ddd, [d] = DDD, ", ToString(m));
	}
}
