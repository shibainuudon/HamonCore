/**
 *	@file	unit_test_multimap_value_comp.cpp
 *
...
GTEST_TEST(MultimapTest, ValueCompTest)
{
	...
	{
		using Map = hamon::multimap<int, char, ModCmp>;
		using Pair = typename Map::value_type;
		Map cont;
		cont = {{1, 'a'}, {2, 'b'}, {3, 'c'}, {4, 'd'}, {5, 'e'}};

		auto comp_func = cont.value_comp();
		const Pair val = {100, 'a'};
		auto it = cont.begin();
		EXPECT_TRUE (comp_func(*it, val));
		EXPECT_FALSE(comp_func(val, *it));
		++it;
		EXPECT_TRUE (comp_func(*it, val));
		EXPECT_FALSE(comp_func(val, *it));
		++it;
		EXPECT_FALSE(comp_func(*it, val));
		EXPECT_FALSE(comp_func(val, *it));
		++it;
		EXPECT_FALSE(comp_func(*it, val));
		EXPECT_TRUE (comp_func(val, *it));
		++it;
		EXPECT_FALSE(comp_func(*it, val));
		EXPECT_TRUE (comp_func(val, *it));
		++it;
