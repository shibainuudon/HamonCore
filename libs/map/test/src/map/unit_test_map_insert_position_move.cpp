/**
 *	@file	unit_test_map_insert_position_move.cpp
 *
 *	@brief	insert のテスト
 *
...
MAP_TEST_CONSTEXPR bool test1()
{
	...
	{
		auto r = v.insert(v.end(), ValueType{Key{1}, T{20}});
		VERIFY(r == hamon::next(v.begin(), 0));
	...
	{
		auto r = v.insert(hamon::next(v.begin(), 1), ValueType{Key{3}, T{10}});
	...
	{
		auto r = v.insert(hamon::next(v.begin(), 2), ValueType{Key{2}, T{40}});
		VERIFY(r == hamon::next(v.begin(), 2));
...
MAP_TEST_CONSTEXPR bool test2()
{
	...
	{
		auto r = v.insert(v.end(), ValueType{3, S1{70, 80}});
		VERIFY(r == hamon::next(v.begin(), 2));
...
GTEST_TEST(MapTest, InsertPositionMoveTest)
{
