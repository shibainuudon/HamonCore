/**
 *	@file	unit_test_map_try_emplace_hint_heterogeneous.cpp
 *
...
MAP_TEST_CONSTEXPR bool test()
{
	...
	{
		Map v;
		static_assert(hamon::is_same<decltype(v.try_emplace(hamon::declval<ConstIterator>(), hamon::declval<int const&>(), hamon::declval<float>())), Iterator>::value, "");
		static_assert(hamon::is_same<decltype(v.try_emplace(hamon::declval<ConstIterator>(), hamon::declval<int&&>(), hamon::declval<float>())), Iterator>::value, "");
		static_assert(!noexcept(v.try_emplace(hamon::declval<ConstIterator>(), hamon::declval<int const&>(), hamon::declval<float>())), "");
		static_assert(!noexcept(v.try_emplace(hamon::declval<ConstIterator>(), hamon::declval<int&&>(), hamon::declval<float>())), "");
	}

	Map v;
	{
		auto r = v.try_emplace(v.end(), 1, 0.5f);
	...
	{
		auto r = v.try_emplace(v.end(), 1, 1.5f);
		VERIFY(r == hamon::next(v.begin(), 0));
	...
	{
		auto r = v.try_emplace(v.end(), 2, 2.5f);
	...
	{
		int const k = 0;
		auto r = v.try_emplace(v.end(), k, 3.5f);
		VERIFY(r == hamon::next(v.begin(), 0));
	...
	{
		int const k = 2;
		auto r = v.try_emplace(v.end(), k, 4.5f);
...
GTEST_TEST(MapTest, TryEmplaceHintHeterogeneousTest)
{
