/**
 *	@file	unit_test_map_upper_bound.cpp
 *
...
MAP_TEST_CONSTEXPR bool test2()
{
	...
	VERIFY(v.upper_bound(Key{4}) == hamon::next(v.begin(), 2));
	VERIFY(v.upper_bound(Key{5}) == hamon::next(v.begin(), 3));
	VERIFY(v.upper_bound(Key{6}) == hamon::next(v.begin(), 3));
...
MAP_TEST_CONSTEXPR bool test4()
{
	...
	VERIFY(v.upper_bound(5) == hamon::next(v.begin(), 2));
	VERIFY(v.upper_bound(6) == hamon::next(v.begin(), 3));
	VERIFY(v.upper_bound(7) == hamon::next(v.begin(), 3));
...
GTEST_TEST(MapTest, UpperBoundTest)
{
