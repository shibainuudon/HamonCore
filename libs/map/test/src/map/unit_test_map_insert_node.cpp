/**
 *	@file	unit_test_map_insert_node.cpp
 *
 *	@brief	insert のテスト
 *
...
MAP_TEST_CONSTEXPR bool test3()
{
...

	{
		...
		VERIFY(r.inserted == true);
		VERIFY(r.position == hamon::next(v1.begin(), 3));
		VERIFY(r.node.empty() == true);
	}
	VERIFY(v1.size() == 4);
	{
...

	{
		...
		VERIFY(r.inserted == false);
		VERIFY(r.position == hamon::next(v1.begin(), 2));
		VERIFY(r.node.empty() == false);
		VERIFY(r.node.key() == 4);
		VERIFY(r.node.mapped() == 20.5f);
	}
	VERIFY(v1.size() == 4);
	{
		auto it = v1.begin();
...
GTEST_TEST(MapTest, InsertNodeTest)
{
