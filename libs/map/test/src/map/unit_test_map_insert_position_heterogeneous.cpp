/**
 *	@file	unit_test_map_insert_position_heterogeneous.cpp
 *
 *	@brief	insert のテスト
 *
 *	template<class P>
...
MAP_TEST_CONSTEXPR bool test1()
{
	...
	{
		auto r = v.insert(v.begin(), std::make_pair(char{1}, 3.5f));
		VERIFY(r == hamon::next(v.begin(), 1));
	...
	// from std::tuple
	{
		auto r = v.insert(hamon::next(v.begin(), 2), std::make_tuple(char{6}, 4.5f));
	...
	// from std::array
	{
		auto r = v.insert(hamon::next(v.begin(), 3), std::array<int, 2>{3, 5});
		VERIFY(r == hamon::next(v.begin(), 3));
	...
	// from hamon::array
	{
		auto r = v.insert(hamon::next(v.begin(), 5), hamon::array<int, 2>{5, 8});
...
GTEST_TEST(MapTest, InsertPositionHeterogeneousTest)
{
...
#if !defined(HAMON_NO_EXCEPTIONS)
	{
		...
		EXPECT_THROW(v.insert(v.begin(), std::pair<int, int>{2, -10}), MayThrow::Exception);
		EXPECT_EQ(1u, v.size());
