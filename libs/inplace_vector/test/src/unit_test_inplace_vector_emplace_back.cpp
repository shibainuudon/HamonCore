/**
 *	@file	unit_test_inplace_vector_emplace_back.cpp
 *
...
GTEST_TEST(InplaceVectorTest, EmplaceBackTest)
{
...
#if !defined(HAMON_NO_EXCEPTIONS)
	{
		...
		EXPECT_THROW(v.emplace_back(1), hamon::bad_alloc);
		EXPECT_TRUE(v.empty());
		EXPECT_EQ(0u, v.size());
	}
	{
		hamon::inplace_vector<int, 1> v;
		EXPECT_NO_THROW(v.emplace_back(10));
		EXPECT_EQ(1u, v.size());
		EXPECT_EQ(10, v[0]);
		EXPECT_THROW(v.emplace_back(20), hamon::bad_alloc);
		EXPECT_EQ(1u, v.size());
		EXPECT_EQ(10, v[0]);
	}
	{
		...
		EXPECT_THROW(v.emplace_back(-10), MayThrow::Exception);
		EXPECT_EQ(1u, v.size());
		EXPECT_EQ(10, v[0].value);
		EXPECT_NO_THROW(v.emplace_back(20));
		EXPECT_EQ(2u, v.size());
		EXPECT_EQ(10, v[0].value);
		...
		EXPECT_EQ(10, v[0].value);
		EXPECT_EQ(20, v[1].value);
		EXPECT_NO_THROW(v.emplace_back(30));
		EXPECT_EQ(3u, v.size());
		EXPECT_EQ(10, v[0].value);
		EXPECT_EQ(20, v[1].value);
	...
	// https://en.cppreference.com/w/cpp/container/inplace_vector/emplace_back
	{
		...
		fauna.emplace_back("CAT", hamon::move(dog));
		{
			...
			EXPECT_EQ("(CAT, DOG), (CAT, DOG), ", ss.str());
		}
