/**
 *	@file	msvc_formatter.hpp
 *
 *	@brief
 */

#ifndef HAMON_FORMAT_DETAIL_MSVC_FORMATTER_HPP
#define HAMON_FORMAT_DETAIL_MSVC_FORMATTER_HPP
...
HAMON_NODISCARD consteval bool _Is_debug_enabled_fmt_type(_Basic_format_arg_type _Ty) {
...
HAMON_NORETURN inline void _Throw_format_error(const char* _Message);
...
public:
    ...
    HAMON_NODISCARD constexpr _ParseContext::iterator _Parse(type_identity_t<_ParseContext&> _Parse_ctx) {
        return hamon::_Fill_align_and_width_formatter_parse(_Specs, _Parse_ctx);
    }

    template <class _FormatContext, class _Func>
    HAMON_NODISCARD constexpr auto _Format(
        _FormatContext& _Format_ctx, const int _Width, _Fmt_align _Default_align, _Func&& _Fn) const {
        return hamon::_Fill_align_and_width_formatter_format(
            _Specs, _Format_ctx, _Width, _Default_align, hamon::forward<_Func>(_Fn));
    }
...
#endif // HAMON_FORMAT_DETAIL_MSVC_FORMATTER_HPP
