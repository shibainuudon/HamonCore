/**
 *	@file	msvc_ranges_tuple_formatter.hpp
 *
 *	@brief	msvc_ranges_tuple_formatter の定義
 */

#ifndef HAMON_FORMAT_DETAIL_msvc_ranges_tuple_formatter_HPP
#define HAMON_FORMAT_DETAIL_msvc_ranges_tuple_formatter_HPP
...
_NODISCARD constexpr auto _To_unsigned_like(const _Ty _Value) noexcept {
    return static_cast<make_unsigned_t<_Ty>>(_Value);
}
...
public:
    ...
    private:
        ...
        template <class _Ty>
        explicit handle(_Ty& _Val) noexcept : _Ptr(hamon::addressof(_Val)), _Format(_Handle_format<_Ty>) {}
    ...
    decltype(auto) _Visit(_Visitor&& _Vis) {
        ...
        case _Basic_format_arg_type::_None:
            return hamon::forward<_Visitor>(_Vis)(_No_state);
        case _Basic_format_arg_type::_Int_type:
            return hamon::forward<_Visitor>(_Vis)(_Int_state);
        case _Basic_format_arg_type::_UInt_type:
            return hamon::forward<_Visitor>(_Vis)(_UInt_state);
        case _Basic_format_arg_type::_Long_long_type:
            return hamon::forward<_Visitor>(_Vis)(_Long_long_state);
        case _Basic_format_arg_type::_ULong_long_type:
            return hamon::forward<_Visitor>(_Vis)(_ULong_long_state);
        case _Basic_format_arg_type::_Bool_type:
            return hamon::forward<_Visitor>(_Vis)(_Bool_state);
        case _Basic_format_arg_type::_Char_type:
            return hamon::forward<_Visitor>(_Vis)(_Char_state);
        case _Basic_format_arg_type::_Float_type:
            return hamon::forward<_Visitor>(_Vis)(_Float_state);
        case _Basic_format_arg_type::_Double_type:
            return hamon::forward<_Visitor>(_Vis)(_Double_state);
        case _Basic_format_arg_type::_Long_double_type:
            return hamon::forward<_Visitor>(_Vis)(_Long_double_state);
        case _Basic_format_arg_type::_Pointer_type:
            return hamon::forward<_Visitor>(_Vis)(_Pointer_state);
        case _Basic_format_arg_type::_CString_type:
            return hamon::forward<_Visitor>(_Vis)(_CString_state);
        case _Basic_format_arg_type::_String_type:
            return hamon::forward<_Visitor>(_Vis)(_String_state);
        case _Basic_format_arg_type::_Custom_type:
            return hamon::forward<_Visitor>(_Vis)(_Custom_state);
        default:
            _STL_REPORT_ERROR("basic_format_arg contains an impossible type");
            int _Dummy{};
            return hamon::forward<_Visitor>(_Vis)(_Dummy);
        }
    }
...
decltype(auto) visit_format_arg(_Visitor&& _Vis, basic_format_arg<_Context> _Arg) {
    return _Arg._Visit(hamon::forward<_Visitor>(_Vis));
}
...
#pragma warning(disable : 6386) // Buffer overrun while writing to '%s' ...
    ...
    void _Store_impl(
        ...
        _CSTD memcpy(_Storage + _Store_index, hamon::addressof(_Val), sizeof(_Ty));
...
#pragma warning(pop)
    ...
    void _Store(const size_t _Arg_index, _Ty&& _Val) noexcept {
        constexpr _Basic_format_arg_type _Arg_type = hamon::_Get_format_arg_type<_Context, _Ty>();
...
private:
    ...
    _NODISCARD static auto _Get_value_from_memory(const unsigned char* const _Val) noexcept {
        ...
        return hamon::bit_cast<_Ty>(_Temp);
    }
...
class _Lazy_locale {
private:
    const std::locale* _Loc = nullptr;

public:
    _Lazy_locale() = default;

    explicit _Lazy_locale(const std::locale& _Loc_) : _Loc(&_Loc_) {}

    explicit _Lazy_locale(const std::locale&&) = delete;

    _NODISCARD std::locale _Get() const {
        if (_Loc) {
            return *_Loc;
        } else {
            return std::locale{};
...
private:
    ...
    constexpr basic_format_context(
        ...
        : _OutputIt(hamon::move(_OutputIt_)), _Args(_Ctx_args), _Loc(_Loc_) {}
...
public:
    ...
    _NODISCARD std::locale locale() {
        return _Loc._Get();
    }
    _NODISCARD iterator out() {
        return hamon::move(_OutputIt);
    }
    void advance_to(iterator _It) {
        _OutputIt = hamon::move(_It);
    }
    ...
    _NODISCARD static constexpr basic_format_context _Make_from(
        ...
        return basic_format_context{hamon::move(_OutputIt_), _Ctx_args, _Loc_};
    }
};
...
struct _Fmt_iterator_flush {
    ...
    static _OutputIt _Flush(const _Ty* const _First, const _Ty* const _Last, _OutputIt _Output) {
        return hamon::copy(_First, _Last, hamon::move(_Output));
    }
};
...
private:
    ...
    void _Flush() {
        ...
        _Output = _Fmt_iterator_flush<_OutputIt>::_Flush(_Data, _End, hamon::move(_Output));
    }

public:
    explicit _Fmt_iterator_buffer(_OutputIt _Out, ptrdiff_t _Size = _Fmt_buffer_size)
        : _Traits(_Size), _Fmt_buffer<_Ty>(_Data, 0, _Fmt_buffer_size), _Output(hamon::move(_Out)) {}
...
public:
    ...
    constexpr _ParseContext::iterator parse(_ParseContext& _Ctx) {
        return hamon::_Range_formatter_parse(_Underlying, _Separator, _Opening_bracket, _Closing_bracket, _Specs, _Ctx);
    }

    template <ranges::input_range _Range, class _FormatContext>
        requires formattable<ranges::range_reference_t<_Range>, _CharT>
              && same_as<remove_cvref_t<ranges::range_reference_t<_Range>>, _Ty>
    _FormatContext::iterator format(_Range&& _Rng, _FormatContext& _Ctx) const {
        return _Format(hamon::forward<_Range>(_Rng), _Ctx);
    }
...
private:
    ...
    _FormatContext::iterator _Format(_Range&& _Rng, _FormatContext& _Ctx) const {
        return hamon::_Range_formatter_format(
            _Underlying, _Separator, _Opening_bracket, _Closing_bracket, _Specs, hamon::forward<_Range>(_Rng), _Ctx);
    }
};

template <class _Rng, class _CharT>
concept _Const_formattable_range =
    ranges::input_range<const _Rng> && formattable<ranges::range_reference_t<const _Rng>, _CharT>;

template <class _Rng, class _CharT>
using _Fmt_maybe_const = conditional_t<_Const_formattable_range<_Rng, _CharT>, const _Rng, _Rng>;

template <range_format _Kind, ranges::input_range _Rng, class _CharT>
struct _Range_default_formatter;

template <ranges::input_range _Rng, class _CharT>
struct _Range_default_formatter<range_format::sequence, _Rng, _CharT> {
private:
    using _Range_type = _Fmt_maybe_const<_Rng, _CharT>;

    range_formatter<remove_cvref_t<ranges::range_reference_t<_Range_type>>, _CharT> _Underlying;
...
private:
    ...
    using _Element_type = remove_cvref_t<ranges::range_reference_t<_Map_type>>;
...
private:
    ...
    range_formatter<remove_cvref_t<ranges::range_reference_t<_Set_type>>, _CharT> _Underlying;

public:
    constexpr _Range_default_formatter() noexcept(is_nothrow_default_constructible_v<
        range_formatter<remove_cvref_t<ranges::range_reference_t<_Set_type>>, _CharT>>) /* strengthened */ {
        _Underlying.set_brackets(_STATICALLY_WIDEN(_CharT, "{"), _STATICALLY_WIDEN(_CharT, "}"));
    }
...
private:
    static_assert(is_same_v<remove_cvref_t<ranges::range_reference_t<_Rng>>, _CharT>,
        "the element type of the formatted range must be the character type used in formatting "
        "(N4981 [format.range.fmtstr]/1)");

    using _Range_type = _Maybe_const<ranges::input_range<const _Rng>, _Rng>;
...
public:
    ...
    _FormatContext::iterator format(_Range_type& _Rx, _FormatContext& _Ctx) const {
        if constexpr (ranges::contiguous_range<_Range_type>) {
            const auto _Size = hamon::_To_unsigned_like(ranges::distance(_Rx));

            if (!hamon::in_range<size_t>(_Size)) {
                _Throw_format_error("Formatted range is too long.");
            }

            const basic_string_view<_CharT> _Str(hamon::to_address(ranges::begin(_Rx)), static_cast<size_t>(_Size));
...
template <ranges::input_range _Rng, _Format_supported_charT _CharT>
...
template <ranges::input_range _Rng, _Format_supported_charT _CharT>
    requires _Formatting_enabled_range<_Rng> && formattable<ranges::range_reference_t<_Rng>, _CharT>
...
protected:
    ...
    _FormatContext::iterator _Format(_FormatContext& _Fmt_ctx, _ArgTypes&... _Args) const {
        ...
        return hamon::_Tuple_formatter_format(
...
public:
    ...
    template <class _ParseContext>
    constexpr _ParseContext::iterator parse(_ParseContext& _Ctx) {
        return hamon::_Tuple_formatter_parse(_Underlying, _Separator, _Opening_bracket, _Closing_bracket, _Specs, _Ctx);
    }
};
...
public:
    template <class _FormatContext>
    _FormatContext::iterator format(_Formatted_type& _Elems, _FormatContext& _Ctx) const {
        return hamon::apply([this, &_Ctx](auto&... _Args) { return this->_Format(_Ctx, _Args...); }, _Elems);
    }
};
...
template <_Format_supported_charT _CharT, class _Ty1, class _Ty2>
    requires (!formattable<_Ty1, _CharT> || !formattable<_Ty2, _CharT>)
          ...
          && (ranges::input_range<pair<_Ty1, _Ty2>>) && _Formatting_enabled_range<pair<_Ty1, _Ty2>>
          && formattable<ranges::range_reference_t<pair<_Ty1, _Ty2>>, _CharT>
...
template <_Format_supported_charT _CharT, class... _Types>
    requires ((!formattable<_Types, _CharT>) || ...)
          // TRANSITION, clang-format, () should be redundant
          && (ranges::input_range<tuple<_Types...>>) && _Formatting_enabled_range<tuple<_Types...>>
          && formattable<ranges::range_reference_t<tuple<_Types...>>, _CharT>
...
#endif // HAMON_FORMAT_DETAIL_msvc_ranges_tuple_formatter_HPP
