...
#ifndef HAMON_FORMAT_DETAIL_IMPL_HPP
#define HAMON_FORMAT_DETAIL_IMPL_HPP

#include <hamon/format/detail/msvc_ranges_tuple_formatter.hpp>
#include <hamon/format/detail/msvc_format_ucd_tables.hpp>
#include <hamon/format/detail/msvc_formatter.hpp>
...
//extern "C" HAMON_NODISCARD __std_win_error __stdcall __std_get_cvt(__std_code_page _Codepage, _Cvtvec* _Pcvt) noexcept;

HAMON_NODISCARD constexpr bool _Is_integral_fmt_type(_Basic_format_arg_type _Ty) {
    return _Ty > _Basic_format_arg_type::_None && _Ty <= _Basic_format_arg_type::_Char_type;
}

HAMON_NODISCARD constexpr bool _Is_arithmetic_fmt_type(_Basic_format_arg_type _Ty) {
...
HAMON_NODISCARD constexpr _Decode_result<wchar_t> _Decode_utf(
...
HAMON_NODISCARD constexpr _Decode_result<char> _Decode_utf(const char* _First, const char* _Last, char32_t& _Val) noexcept {
    _STL_INTERNAL_CHECK(_First < _Last);
...
HAMON_NODISCARD constexpr _Decode_result<char32_t> _Decode_utf(
...
public:
    ...
    HAMON_NODISCARD constexpr value_type operator*() const noexcept {
        return _Val;
    }

    HAMON_NODISCARD constexpr const _CharT* _Position() const noexcept {
        return _First;
    }

    HAMON_NODISCARD constexpr bool operator==(default_sentinel_t) const noexcept {
        return _First == _Last;
    }
    HAMON_NODISCARD constexpr bool operator==(const _Unicode_codepoint_iterator& _Other) const noexcept {
...
public:
    HAMON_NODISCARD constexpr bool operator==(const _GB11_LeftHand_regex&) const noexcept = default;

    HAMON_NODISCARD constexpr bool _Match(
...
public:
    ...
    HAMON_NODISCARD constexpr bool operator==(const _Grapheme_break_property_iterator&) const noexcept = default;
...
template <class _CharT>
HAMON_NODISCARD constexpr const _CharT* _Parse_nonnegative_integer(
    const _CharT* _First, const _CharT* _Last, unsigned int& _Value) {
    _STL_INTERNAL_CHECK(_First != _Last && '0' <= *_First && *_First <= '9');

    constexpr auto _Max_int = static_cast<unsigned int>(hamon::numeric_limits<int>::max());	//hamon::_Max_limit<int>());
...
template <class _CharT>
HAMON_NODISCARD constexpr const _CharT* _Parse_nonnegative_integer(const _CharT* _First, const _CharT* _Last, int& _Value) {
...
HAMON_NODISCARD constexpr const _CharT* _Parse_arg_id(
...
HAMON_NODISCARD constexpr bool _Is_execution_charset_self_synchronizing() {
...
HAMON_NODISCARD constexpr int _Unicode_width_estimate(const char32_t _Ch) noexcept {
    ...
    if (_Ch < _Linear_search_threshold) {
        ...
        const ptrdiff_t _Upper_bound_index =
            hamon::upper_bound(_Width_estimate_intervals_v2, hamon::end(_Width_estimate_intervals_v2), _Ch)
...
protected:
    ...
    HAMON_NODISCARD int _Double_byte_encoding_code_units_in_next_character(
...
private:
    HAMON_NODISCARD static constexpr int _Utf8_code_units_in_next_character(
...
public:
    HAMON_NODISCARD constexpr const char* _Find_encoded(const char* _First, const char* const _Last, const char _Val) const {
        ...
        if constexpr (_Statically_Utf8) {
            return hamon::find/*_Find_unchecked*/(_First, _Last, _Val);
        } else {
    ...
    HAMON_NODISCARD constexpr int _Units_in_next_character(
...
#if _HAS_CXX23
    HAMON_NODISCARD _Decode_result<char> _Decode(const char* const _First, const char* const _Last, char32_t& _Val) const {
        _STL_INTERNAL_CHECK(_First < _Last);
...
public:
    HAMON_NODISCARD constexpr const wchar_t* _Find_encoded(
...
#if _HAS_CXX23
    HAMON_NODISCARD constexpr _Decode_result<wchar_t> _Decode(
...
template <class _CharT>
HAMON_NODISCARD constexpr _Fmt_codec<_CharT> _Get_fmt_codec() {
    return {};
}

template <class _CharT>
    requires (is_same_v<_CharT, char> && !_Is_execution_charset_self_synchronizing())
HAMON_NODISCARD const _Fmt_codec<_CharT>& _Get_fmt_codec() {
    using _CodecType = _Fmt_codec<_CharT>;

    static std::once_flag _Flag;
    alignas(_CodecType) static unsigned char _Storage[sizeof(_CodecType)];

    std::call_once(_Flag, [] { hamon::construct_at(reinterpret_cast<_CodecType*>(&_Storage)); });

    return *hamon::launder(reinterpret_cast<const _CodecType*>(&_Storage));
}

template <class _CharT, _Parse_align_callbacks<_CharT> _Callbacks_type>
HAMON_NODISCARD constexpr const _CharT* _Parse_align(
    const _CharT* _First, const _CharT* _Last, _Callbacks_type&& _Callbacks) {
    // align and fill
...
HAMON_NODISCARD constexpr const _CharT* _Parse_width(
    const _CharT* _First, const _CharT* _Last, _Callbacks_type&& _Callbacks) {
    _STL_INTERNAL_CHECK(_First != _Last);
...
HAMON_NODISCARD constexpr const _CharT* _Parse_precision(
    const _CharT* _First, const _CharT* _Last, _Callbacks_type&& _Callbacks) {
    ++_First;
    _CharT _Ch = '\0';
    if (_First != _Last) {
...
HAMON_NODISCARD constexpr const _CharT* _Parse_format_specs(
    const _CharT* _First, const _CharT* _Last, _Callbacks_type&& _Callbacks) {
    if (_First == _Last || *_First == '}') {
        return _First;
    }
...
HAMON_NODISCARD constexpr const _CharT* _Parse_replacement_field(
...
public:
    ...
    constexpr void _On_fill(const basic_string_view<_CharT> _Sv) {
        if (_Sv.size() > hamon::size(_Specs._Fill)) {
            _Throw_format_error("Invalid fill (too long).");
        }

        const auto _Pos = hamon::copy/*_Copy_unchecked*/(_Sv._Unchecked_begin(), _Sv._Unchecked_end(), _Specs._Fill);
        hamon::fill(_Pos, hamon::end(_Specs._Fill), _CharT{});
...
HAMON_NODISCARD constexpr basic_format_arg<_Context> _Get_arg(const _Context& _Ctx, const size_t _Arg_id) {
...
public:
    template <class _Ty>
    HAMON_NODISCARD constexpr unsigned long long operator()(const _Ty _Value) const {
...
HAMON_NODISCARD constexpr int _Get_dynamic_specs(const _FormatArg _Arg) {
    _STL_INTERNAL_STATIC_ASSERT(_Is_any_of_v<_Handler, _Width_checker, _Precision_checker>);
    const unsigned long long _Val = hamon::visit_format_arg(_Handler{}, _Arg);
    if (!hamon::in_range<int>(_Val)) {
        _Throw_format_error("Number is too big.");
    }
...
private:
    ...
    HAMON_NODISCARD constexpr basic_format_arg<_Context> _Get_arg(_Auto_id_tag) {
        return hamon::_Get_arg(_Ctx, _Parse_ctx.next_arg_id());
    }

    HAMON_NODISCARD constexpr basic_format_arg<_Context> _Get_arg(const size_t _Arg_id) {
        _Parse_ctx.check_arg_id(_Arg_id);
        return hamon::_Get_arg(_Ctx, _Arg_id);
    }
};
...
private:
    ...
    HAMON_NODISCARD static constexpr int _Verify_dynamic_arg_index_in_range(const size_t _Idx) {
        if (!hamon::in_range<int>(_Idx)) {
...
public:
    ...
    constexpr void _On_type(_CharT _Type) {
        if (_Type < 0 || _Type > hamon::numeric_limits<signed char>::max()) {
...
public:
    ...
    HAMON_NODISCARD size_t _Count() const noexcept {
        return static_cast<size_t>(_Count_);
    }

    HAMON_NODISCARD size_t _Limit(const size_t _Size) noexcept {
...
using format_context  = _Default_format_context<char>;
using wformat_context = _Default_format_context<wchar_t>;

_FMT_P2286_BEGIN
template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Fmt_write(_OutputIt _Out, monostate) {
    _STL_INTERNAL_CHECK(false);
    return _Out;
}
...
HAMON_NODISCARD _OutputIt _Fmt_write(_OutputIt _Out, _Arithmetic _Value);

template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Fmt_write(_OutputIt _Out, bool _Value);

template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Fmt_write(_OutputIt _Out, _CharT _Value);

template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Fmt_write(_OutputIt _Out, const void* _Value);

template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Fmt_write(_OutputIt _Out, const _CharT* _Value);

template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Fmt_write(_OutputIt _Out, basic_string_view<_CharT> _Value);

template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Widen_and_copy(const char* _First, const char* const _Last, _OutputIt _Out) {
...
template <class _CharT, class _OutputIt, class _Arithmetic>
    requires (is_arithmetic_v<_Arithmetic> && !_CharT_or_bool<_Arithmetic, _CharT>)
HAMON_NODISCARD _OutputIt _Fmt_write(_OutputIt _Out, const _Arithmetic _Value) {
    // TRANSITION, Reusable buffer
    char _Buffer[_Format_min_buffer_length];
    char* _End = _Buffer;

    if constexpr (is_floating_point_v<_Arithmetic>) {
        if ((hamon::isnan)(_Value)) {
            if ((hamon::signbit)(_Value)) {
                *_End++ = '-';
            }
    ...
    if (_End == _Buffer) {
        const auto/*to_chars_result*/ _Result = hamon::to_chars(_Buffer, hamon::end(_Buffer), _Value);
        _STL_INTERNAL_CHECK(_Result.ec == errc{});
        _End = _Result.ptr;
    }

    return _Widen_and_copy<_CharT>(_Buffer, _End, hamon::move(_Out));
}

template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Fmt_write(_OutputIt _Out, const bool _Value) {
    if constexpr (is_same_v<_CharT, wchar_t>) {
        return _Fmt_write(hamon::move(_Out), _Value ? L"true" : L"false");
    } else {
        return _Fmt_write(hamon::move(_Out), _Value ? "true" : "false");
    }
}
...
template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Fmt_write(_OutputIt _Out, const void* const _Value) {
    // TRANSITION, Reusable buffer
    char _Buffer[_Format_min_buffer_length];
    const auto [_End, _Ec] = hamon::to_chars(_Buffer, hamon::end(_Buffer), reinterpret_cast<uintptr_t>(_Value), 16);
...
template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Fmt_write(_OutputIt _Out, const basic_string_view<_CharT> _Value) {
    return ranges::copy(_Value, hamon::move(_Out)).out;
}

template <class _OutputIt, class _Specs_type, class _Func>
HAMON_NODISCARD _OutputIt _Write_aligned(
    ...
    for (; _Fill_left > 0; --_Fill_left) {
        _Out = ranges::copy(_Fill_char, hamon::move(_Out)).out;
    }

    _Out = _Fn(hamon::move(_Out));
...
HAMON_NODISCARD constexpr string_view _Get_integral_prefix(const char _Type, const _Integral _Value) noexcept {
    switch (_Type) {
    case 'b':
        return "0b"_sv;
    case 'B':
...
HAMON_NODISCARD _OutputIt _Write_sign(_OutputIt _Out, const _Fmt_sign _Sgn, const bool _Is_negative) {
    if (_Is_negative) {
        *_Out++ = '-';
    } else {
        switch (_Sgn) {
...
HAMON_NODISCARD constexpr bool _In_bounds(const _Ty _Value) {
    return hamon::in_range<_Make_standard_integer<_CharT>>(static_cast<_Make_standard_integer<_Ty>>(_Value));
}

HAMON_NODISCARD inline int _Count_separators(size_t _Digits, const string_view _Groups) {
    if (_Groups.empty()) {
        return 0;
    }
...
template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Write_separated_integer(const char* _First, const char* const _Last, const string_view _Groups,
    ...
    _Out   = _Widen_and_copy<_CharT>(_First, _Last - _Grouped, hamon::move(_Out));
    ...
    for (; _Separators > 0; --_Separators) {
        ...
        *_Out++ = _Separator;
        _Out    = _Widen_and_copy<_CharT>(_First, _First + *_Group_it, hamon::move(_Out));
        _First += *_Group_it;
    }
    _STL_INTERNAL_CHECK(_First == _Last);
    return _Out;
...
template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Fmt_write(_OutputIt _Out, monostate, const _Basic_format_specs<_CharT>&, _Lazy_locale) {
    _STL_INTERNAL_CHECK(false);
    return _Out;
}

template <class _CharT, class _OutputIt, integral _Integral>
HAMON_NODISCARD _OutputIt _Write_integral(
    _OutputIt _Out, _Integral _Value, _Basic_format_specs<_CharT> _Specs, _Lazy_locale _Locale);

#if _HAS_CXX23
template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Write_escaped(
...
HAMON_NODISCARD _OutputIt _Fmt_write(
    _OutputIt _Out, _Integral _Value, const _Basic_format_specs<_CharT>& _Specs, _Lazy_locale _Locale);

template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Fmt_write(_OutputIt _Out, bool _Value, _Basic_format_specs<_CharT> _Specs, _Lazy_locale _Locale);
...
HAMON_NODISCARD _OutputIt _Write_integral(
    ...
    if (_Specs._Type == 'c') {
        ...
        _Specs._Alt = false;
        return _Fmt_write(hamon::move(_Out), static_cast<_CharT>(_Value), _Specs, _Locale);
    }
    ...
    char _Buffer[65];
    const auto [_End, _Ec] = hamon::to_chars(_Buffer, hamon::end(_Buffer), _Value, _Base);
    _STL_INTERNAL_CHECK(_Ec == errc{});
    ...
    if (_Specs._Localized) {
        _Groups     = std::use_facet<std::numpunct<_CharT>>(_Locale._Get()).grouping();
    ...
    auto _Writer                     = [&, _End = _End](_OutputIt _Out) {
        _Out = _Write_sign(hamon::move(_Out), _Specs._Sgn, _Value < _Integral{0});
        _Out = _Widen_and_copy<_CharT>(_Prefix.data(), _Prefix.data() + _Prefix.size(), hamon::move(_Out));
        if (_Write_leading_zeroes && _Width < _Specs._Width) {
            _Out = ranges::fill_n(hamon::move(_Out), _Specs._Width - _Width, _CharT{'0'});
        }

        if (_Separators > 0) {
            return _Write_separated_integer(_Buffer_start, _End, _Groups,
                                    std::use_facet<std::numpunct<_CharT>>(_Locale._Get()).thousands_sep(), //
                                    _Separators, hamon::move(_Out));
        }
        return _Widen_and_copy<_CharT>(_Buffer_start, _End, hamon::move(_Out));
    };

    if (_Write_leading_zeroes) {
        return _Writer(hamon::move(_Out));
    }

    return _Write_aligned(hamon::move(_Out), _Width, _Specs, _Fmt_align::_Right, _Writer);
}
#pragma warning(pop)
...
HAMON_NODISCARD _OutputIt _Fmt_write(
    ...
    return _Write_integral(hamon::move(_Out), _Value, _Specs, _Locale);
}
...
HAMON_NODISCARD _OutputIt _Fmt_write(
    ...
    if (_Specs._Type != '\0' && _Specs._Type != 's') {
        return _Write_integral(hamon::move(_Out), static_cast<unsigned char>(_Value), _Specs, _Locale);
    }
    ...
    if (_Specs._Localized) {
        ...
        const auto& _Facet = std::use_facet<std::numpunct<_CharT>>(_Locale._Get());
        return _Fmt_write(hamon::move(_Out),
    ...
    if constexpr (is_same_v<_CharT, wchar_t>) {
        return _Fmt_write(hamon::move(_Out), _Value ? L"true" : L"false", _Specs, _Locale);
    } else {
        return _Fmt_write(hamon::move(_Out), _Value ? "true" : "false", _Specs, _Locale);
    }
}
...
HAMON_NODISCARD _OutputIt _Fmt_write(
    ...
    if (_Specs._Type != '\0' && _Specs._Type != 'c'
        ...
        return _Write_integral(hamon::move(_Out), static_cast<make_unsigned_t<_CharT>>(_Value), _Specs, _Locale);
    }
...
#if _HAS_CXX23
    if (_Specs._Type == '?') {
        return _Write_escaped(hamon::move(_Out), basic_string_view<_CharT>{&_Value, 1}, _Specs, '\'');
    }
#endif // _HAS_CXX23

    return _Fmt_write(hamon::move(_Out), basic_string_view<_CharT>{&_Value, 1}, _Specs, _Locale);
}
...
HAMON_NODISCARD _OutputIt _Fmt_write(
    ...
    const auto _Is_negative = (hamon::signbit)(_Value);
    ...
    if ((hamon::isnan)(_Value)) {
        ...
        if (_Precision == -1) {
            if (_None_type) {
                _Result = hamon::to_chars(_Buffer, hamon::end(_Buffer), _Value);
            } else {
                _Result = hamon::to_chars(_Buffer, hamon::end(_Buffer), _Value, _Format);
            }
        } else {
            _Result = hamon::to_chars(_Buffer, hamon::end(_Buffer), _Value, _Format, _Precision);
        }
    ...
    const auto _Is_finite = (hamon::isfinite)(_Value);
    ...
    if (_Is_finite) {
        if (_Specs._Alt || _Specs._Localized) {
            ...
            _Integral_end = (hamon::min)(_Radix_point, _Exponent_start);
        ...
        case chars_format::general:
            if (_Specs._Alt && (_Specs._Type == 'g' || _Specs._Type == 'G')) {
                ...
                if (_Exponent_start == _Result.ptr && hamon::abs(_Value) < 1.0 && _Value != 0.0) {
                    for (auto _It = _Buffer_start; _It < _Result.ptr; ++_It) {
                        if (*_It == '0') {
                            ++_Zeroes_to_append;
                        } else if (*_It != '.') {
    ...
    auto _Writer = [&](_OutputIt _Out) {
        _Out = _Write_sign(hamon::move(_Out), _Sgn, _Is_negative);
        ...
        if (_Specs._Localized) {
            ...
            _Out = _Write_separated_integer(
                _Buffer_start, _Integral_end, _Groups, _Facet.thousands_sep(), _Separators, hamon::move(_Out));
        ...
        _Out = _Widen_and_copy<_CharT>(_Buffer_start, _Exponent_start, hamon::move(_Out));
        ...
        return _Widen_and_copy<_CharT>(_Exponent_start, _Result.ptr, hamon::move(_Out));
    };
...
HAMON_NODISCARD _OutputIt _Fmt_write(
    ...
    const bool _Write_leading_zeroes = _Specs._Leading_zero && _Specs._Alignment == _Fmt_align::_None;
    auto _Writer                     = [&](_OutputIt _Out) {
        _Out = ranges::copy(_Prefix, _Prefix + 2, hamon::move(_Out)).out;
        if (_Write_leading_zeroes && _Width < _Specs._Width) {
            _Out = ranges::fill_n(hamon::move(_Out), _Specs._Width - _Width, _CharT{'0'});
        }
        return hamon::_Widen_and_copy<_CharT>(_Buffer, _End, hamon::move(_Out));
    };
    ...
    return hamon::_Write_aligned(hamon::move(_Out), _Width, _Specs, _Fmt_align::_Right, _Writer);
}

template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Fmt_write(
    _OutputIt _Out, const _CharT* _Value, const _Basic_format_specs<_CharT>& _Specs, _Lazy_locale _Locale) {
    return _Fmt_write(hamon::move(_Out), basic_string_view<_CharT>{_Value}, _Specs, _Locale);
}
...
public:
    ...
    HAMON_NODISCARD constexpr bool operator==(const _Measure_string_prefix_iterator_utf&) const noexcept = default;
    ...
    HAMON_NODISCARD constexpr value_type operator*() const {
    ...
    HAMON_NODISCARD constexpr const _CharT* _Position() const {
        return _WrappedIter._Position();
    }
};
...
public:
    ...
    HAMON_NODISCARD bool operator==(default_sentinel_t) const noexcept {
        return _First == _Last;
    }
    HAMON_NODISCARD bool operator==(const _Measure_string_prefix_iterator_legacy& _Rhs) const noexcept {
    ...
    HAMON_NODISCARD value_type operator*() const noexcept {
        return _Units;
    }

    HAMON_NODISCARD const char* _Position() const noexcept {
        return _First;
    }
};
...
template <class _CharT>
HAMON_NODISCARD const _CharT* _Measure_string_prefix(const basic_string_view<_CharT> _Value, int& _Width) {
    ...
    constexpr auto _Max_int = hamon::numeric_limits<int>::max();
...
#if _HAS_CXX23
HAMON_NODISCARD constexpr bool _Is_printable(char32_t _Val) {
    return __printable_property_data._Get_property_for_codepoint(_Val) == __printable_property_values::_Yes_value;
}

HAMON_NODISCARD constexpr bool _Is_grapheme_extend(char32_t _Val) {
...
template <class _CharT, class _OutputIt>
HAMON_NODISCARD _OutputIt _Write_escaped(_OutputIt _Out, basic_string_view<_CharT> _Value, char _Delim) {
    ...
    *_Out++ = static_cast<_CharT>(_Delim);
    ...
    while (_First != _Last) {
        ...
        if (_Ch == static_cast<_CharT>('\t')) {
            _Out                    = _Fmt_write(hamon::move(_Out), _STATICALLY_WIDEN(_CharT, R"(\t)"));
            ...
            _Out                    = _Fmt_write(hamon::move(_Out), _STATICALLY_WIDEN(_CharT, R"(\r)"));
            _Escape_grapheme_extend = true;
            ++_First;
            ...
            *_Out++                 = static_cast<_CharT>(_Delim);
            ...
            if (_Is_usv) {
                ...
                if (_Needs_escape) {
                    _Out = _Fmt_write(hamon::move(_Out), _STATICALLY_WIDEN(_CharT, R"(\u{)"));

                    const auto [_End, _Ec] =
                        hamon::to_chars(_Buffer, hamon::end(_Buffer), static_cast<uint32_t>(_Decoded_ch), 16);
                    _STL_INTERNAL_CHECK(_Ec == errc{});

                    _Out = _Widen_and_copy<_CharT>(_Buffer, _End, hamon::move(_Out));

                    *_Out++                 = static_cast<_CharT>('}');
                    _Escape_grapheme_extend = true;
                } else {
                    _Out                    = hamon::copy/*_Copy_unchecked*/(_First, _Next, hamon::move(_Out));
                ...
                for (; _First != _Next; ++_First) {
                    _Out = _Fmt_write(hamon::move(_Out), _STATICALLY_WIDEN(_CharT, R"(\x{)"));

                    const auto [_End, _Ec] =
                        hamon::to_chars(_Buffer, hamon::end(_Buffer), static_cast<make_unsigned_t<_CharT>>(*_First), 16);
                    _STL_INTERNAL_CHECK(_Ec == errc{});
...
HAMON_NODISCARD _OutputIt _Write_escaped(
    ...
    if (_Specs._Precision < 0 && _Specs._Width <= 0) {
        return _Write_escaped(hamon::move(_Out), _Value, _Delim);
    }
    ...
    return _Write_aligned(hamon::move(_Out), _Width, _Specs, _Fmt_align::_Left, [&_Temp, _Last](_OutputIt _Out) {
        return _Fmt_write(hamon::move(_Out), basic_string_view<_CharT>{_Temp.data(), _Last});
    });
}
#endif // _HAS_CXX23
...
#if _HAS_CXX23
    if (_Specs._Type == '?') {
        return _Write_escaped(hamon::move(_Out), _Value, _Specs, '"');
    }
#endif // _HAS_CXX23

    if (_Specs._Precision < 0 && _Specs._Width <= 0) {
        return _Fmt_write(hamon::move(_Out), _Value);
    }

    int _Width          = _Specs._Precision;
    const _CharT* _Last = _Measure_string_prefix(_Value, _Width);

    return _Write_aligned(hamon::move(_Out), _Width, _Specs, _Fmt_align::_Left,
        [=](_OutputIt _Out) { return _Fmt_write(hamon::move(_Out), basic_string_view<_CharT>{_Value.data(), _Last}); });
}
...
struct _Default_arg_formatter {
    ...
    _OutputIt operator()(_Ty _Val) && {
        return _Fmt_write<_CharT>(hamon::move(_Out), _Val);
    }

    _OutputIt operator()(basic_format_arg<_Context>::handle _Handle) && {
        auto _Format_ctx = _Context::_Make_from(hamon::move(_Out), _Args, _Loc);
...
struct _Format_handler {
    ...
    explicit _Format_handler(_OutputIt _Out, basic_string_view<_CharT> _Str, basic_format_args<_Context> _Format_args,
        ...
        : _Parse_context(_Str), _Ctx(_Context::_Make_from(hamon::move(_Out), _Format_args, _Loc)) {}

    void _On_text(const _CharT* _First, const _CharT* _Last) {
        _Ctx.advance_to(ranges::copy/*_Copy_unchecked*/(_First, _Last, _Ctx.out()).out);
    }
    ...
    void _On_replacement_field(const size_t _Id, const _CharT* _Last) {
        ...
        _Ctx.advance_to(hamon::visit_format_arg(_Default_arg_formatter<_OutputIt, _CharT>{_Ctx.out(), _Ctx._Get_args(),
    ...
    const _CharT* _On_format_specs(const size_t _Id, const _CharT* _First, const _CharT* _Last) {
        ...
        _Ctx.advance_to(hamon::visit_format_arg(
            _Arg_formatter<_OutputIt, _CharT>{._Ctx = hamon::addressof(_Ctx), ._Specs = hamon::addressof(_Specs)}, _Arg));
        return _First;
    }
};
...
_FormatContext::iterator _Formatter_base_format(
    ...
    return hamon::visit_format_arg(
        _Arg_formatter<typename _FormatContext::iterator, _CharT>{
            ._Ctx = hamon::addressof(_FormatCtx), ._Specs = hamon::addressof(_Format_specs)},
    ...
    consteval basic_format_string(const _Ty& _Str_val) : _Str(_Str_val) {
        if constexpr (!is_same_v<_CharT, char> || _Is_execution_charset_self_synchronizing()) {
            ...
            constexpr _Basic_format_arg_type _Arg_types[_Num_args > 0 ? _Num_args : 1] = {
                hamon::_Get_format_arg_type<_Context, _Args>()...};

            _Parse_format_string(_Str, _Format_checker<_CharT, remove_cvref_t<_Args>...>{_Str, _Arg_types});
        }
    }

    HAMON_NODISCARD constexpr basic_string_view<_CharT> get() const noexcept {
        return _Str;
    }
...
HAMON_NODISCARD auto make_format_args(_Args&... _Vals) {
...
template <class... _Args>
HAMON_NODISCARD auto make_wformat_args(_Args&... _Vals) {
    return hamon::make_format_args<wformat_context>(_Vals...);
}
...
_OutputIt _Format_to_it(_OutputIt _Out, const basic_string_view<_CharT> _Fmt, const basic_format_args<_Context> _Args,
    ...
    if constexpr (is_same_v<_OutputIt, _Fmt_it_char>) {
        ...
        _Fmt_iterator_buffer<_OutputIt, _CharT> _Buf(hamon::move(_Out));
...
_OutputIt vformat_to(_OutputIt _Out, const string_view _Fmt, const format_args _Args) {
    return _Format_to_it(hamon::move(_Out), _Fmt, _Args, _Lazy_locale{});
}
...
template <output_iterator<const char&> _OutputIt>
_OutputIt vformat_to(_OutputIt _Out, const std::locale& _Loc, const string_view _Fmt, const format_args _Args) {
    return _Format_to_it(hamon::move(_Out), _Fmt, _Args, _Lazy_locale{_Loc});
}

template <output_iterator<const wchar_t&> _OutputIt>
_OutputIt vformat_to(_OutputIt _Out, const std::locale& _Loc, const wstring_view _Fmt, const wformat_args _Args) {
    return _Format_to_it(hamon::move(_Out), _Fmt, _Args, _Lazy_locale{_Loc});
}
...
_OutputIt format_to(_OutputIt _Out, const format_string<_Types...> _Fmt, _Types&&... _Args) {
    return hamon::vformat_to(hamon::move(_Out), _Fmt.get(), hamon::make_format_args(_Args...));
}
...
_OutputIt format_to(_OutputIt _Out, const wformat_string<_Types...> _Fmt, _Types&&... _Args) {
    return hamon::vformat_to(hamon::move(_Out), _Fmt.get(), hamon::make_wformat_args(_Args...));
}

template <output_iterator<const char&> _OutputIt, class... _Types>
_OutputIt format_to(_OutputIt _Out, const std::locale& _Loc, const format_string<_Types...> _Fmt, _Types&&... _Args) {
    return hamon::vformat_to(hamon::move(_Out), _Loc, _Fmt.get(), hamon::make_format_args(_Args...));
}

template <output_iterator<const wchar_t&> _OutputIt, class... _Types>
_OutputIt format_to(_OutputIt _Out, const std::locale& _Loc, const wformat_string<_Types...> _Fmt, _Types&&... _Args) {
    return hamon::vformat_to(hamon::move(_Out), _Loc, _Fmt.get(), hamon::make_wformat_args(_Args...));
}

template <int = 0> // improves throughput, see GH-2329
HAMON_NODISCARD string vformat(const string_view _Fmt, const format_args _Args) {
    string _Str;
    _Str.reserve(_Fmt.size() + _Args._Estimate_required_capacity());
    hamon::vformat_to(back_insert_iterator{_Str}, _Fmt, _Args);
    return _Str;
}

template <int = 0> // improves throughput, see GH-2329
HAMON_NODISCARD wstring vformat(const wstring_view _Fmt, const wformat_args _Args) {
    wstring _Str;
    _Str.reserve(_Fmt.size() + _Args._Estimate_required_capacity());
    hamon::vformat_to(back_insert_iterator{_Str}, _Fmt, _Args);
    return _Str;
...
template <int = 0> // improves throughput, see GH-2329
HAMON_NODISCARD string vformat(const std::locale& _Loc, const string_view _Fmt, const format_args _Args) {
    string _Str;
    _Str.reserve(_Fmt.size() + _Args._Estimate_required_capacity());
    hamon::vformat_to(back_insert_iterator{_Str}, _Loc, _Fmt, _Args);
    return _Str;
}

template <int = 0> // improves throughput, see GH-2329
HAMON_NODISCARD wstring vformat(const std::locale& _Loc, const wstring_view _Fmt, const wformat_args _Args) {
    wstring _Str;
    _Str.reserve(_Fmt.size() + _Args._Estimate_required_capacity());
    hamon::vformat_to(back_insert_iterator{_Str}, _Loc, _Fmt, _Args);
    return _Str;
...
HAMON_NODISCARD string format(const format_string<_Types...> _Fmt, _Types&&... _Args) {
    return hamon::vformat(_Fmt.get(), hamon::make_format_args(_Args...));
}

template <class... _Types>
HAMON_NODISCARD wstring format(const wformat_string<_Types...> _Fmt, _Types&&... _Args) {
    return hamon::vformat(_Fmt.get(), hamon::make_wformat_args(_Args...));
}

template <class... _Types>
HAMON_NODISCARD string format(const std::locale& _Loc, const format_string<_Types...> _Fmt, _Types&&... _Args) {
    return hamon::vformat(_Loc, _Fmt.get(), hamon::make_format_args(_Args...));
}

template <class... _Types>
HAMON_NODISCARD wstring format(const std::locale& _Loc, const wformat_string<_Types...> _Fmt, _Types&&... _Args) {
    return hamon::vformat(_Loc, _Fmt.get(), hamon::make_wformat_args(_Args...));
}
_FMT_P2286_END
...
format_to_n_result<_OutputIt> format_to_n(
    ...
    _Fmt_iterator_buffer<_OutputIt, char, _Fmt_fixed_buffer_traits> _Buf(hamon::move(_Out), _Max);
    hamon::vformat_to(_Fmt_it{_Buf}, _Fmt.get(), hamon::make_format_args(_Args...));
...
format_to_n_result<_OutputIt> format_to_n(
    ...
    _Fmt_iterator_buffer<_OutputIt, wchar_t, _Fmt_fixed_buffer_traits> _Buf(hamon::move(_Out), _Max);
    hamon::vformat_to(_Fmt_wit{_Buf}, _Fmt.get(), hamon::make_wformat_args(_Args...));
    return {.out = _Buf._Out(), .size = _Buf._Count()};
}

template <output_iterator<const char&> _OutputIt, class... _Types>
format_to_n_result<_OutputIt> format_to_n(_OutputIt _Out, const iter_difference_t<_OutputIt> _Max, const std::locale& _Loc,
    const format_string<_Types...> _Fmt, _Types&&... _Args) {
    _Fmt_iterator_buffer<_OutputIt, char, _Fmt_fixed_buffer_traits> _Buf(hamon::move(_Out), _Max);
    hamon::vformat_to(_Fmt_it{_Buf}, _Loc, _Fmt.get(), hamon::make_format_args(_Args...));
    return {.out = _Buf._Out(), .size = _Buf._Count()};
}
...
format_to_n_result<_OutputIt> format_to_n(_OutputIt _Out, const iter_difference_t<_OutputIt> _Max, const std::locale& _Loc,
    ...
    _Fmt_iterator_buffer<_OutputIt, wchar_t, _Fmt_fixed_buffer_traits> _Buf(hamon::move(_Out), _Max);
    hamon::vformat_to(_Fmt_wit{_Buf}, _Loc, _Fmt.get(), hamon::make_wformat_args(_Args...));
    return {.out = _Buf._Out(), .size = _Buf._Count()};
}

template <class... _Types>
HAMON_NODISCARD size_t formatted_size(const format_string<_Types...> _Fmt, _Types&&... _Args) {
...
template <class... _Types>
HAMON_NODISCARD size_t formatted_size(const wformat_string<_Types...> _Fmt, _Types&&... _Args) {
...
template <class... _Types>
HAMON_NODISCARD size_t formatted_size(const std::locale& _Loc, const format_string<_Types...> _Fmt, _Types&&... _Args) {
    _Fmt_counting_buffer<char> _Buf;
    hamon::vformat_to(_Fmt_it{_Buf}, _Loc, _Fmt.get(), hamon::make_format_args(_Args...));
    return _Buf._Count();
}
...
template <class... _Types>
HAMON_NODISCARD size_t formatted_size(const std::locale& _Loc, const wformat_string<_Types...> _Fmt, _Types&&... _Args) {
    _Fmt_counting_buffer<wchar_t> _Buf;
    hamon::vformat_to(_Fmt_wit{_Buf}, _Loc, _Fmt.get(), hamon::make_wformat_args(_Args...));
    return _Buf._Count();
}
...
#if _HAS_CXX23
template <class _CharT>
HAMON_NODISCARD int _Measure_display_width(const basic_string_view<_CharT> _Value) {
    int _Width = -1;
...
template <class _CharT>
HAMON_NODISCARD constexpr const _CharT* _Parse_range_specs(
...
HAMON_NODISCARD constexpr _ParseContext::iterator _Range_formatter_parse(formatter<_Ty, _CharT>& _Underlying,
    ...
    auto _It = hamon::_Parse_range_specs(_Ctx._Unchecked_begin(), _Ctx._Unchecked_end(), _Callback);
...
template <class _Ty, class _CharT, ranges::input_range _Range, class _FormatContext>
void _Range_formatter_format_as_sequence(const formatter<_Ty, _CharT>& _Underlying,
    ...
    _Ctx.advance_to(hamon::_Fmt_write(_Ctx.out(), _Opening_bracket));
    bool _Separate   = false;
    auto _Iter       = ranges::begin(_Rng);
    const auto _Sent = ranges::end(_Rng);
    for (; _Iter != _Sent; ++_Iter) {
        auto&& _Elem = *_Iter;
        if (_Separate) {
            _Ctx.advance_to(hamon::_Fmt_write(_Ctx.out(), _Separator));
        }
    ...
    (void) hamon::_Fmt_write(_Ctx.out(), _Closing_bracket);
}

template <class _CharT, ranges::input_range _Range, class _FormatContext>
void _Range_formatter_format_as_string(_Range&& _Rng, _FormatContext& _Ctx, const bool _Debug) {
    if constexpr (ranges::contiguous_range<_Range>) {
        const auto _Size = hamon::_To_unsigned_like(ranges::distance(_Rng));
        ...
        const basic_string_view<_CharT> _Str(hamon::to_address(ranges::begin(_Rng)), static_cast<size_t>(_Size));
...
template <class _Ty, class _CharT, ranges::input_range _Range, class _FormatContext>
HAMON_NODISCARD _FormatContext::iterator _Range_formatter_format(const formatter<_Ty, _CharT>& _Underlying,
    const basic_string_view<_CharT> _Separator, const basic_string_view<_CharT> _Opening_bracket,
    ...
    if (_Specs._Dynamic_width_index >= 0) {
        _Format_specs._Width =
            hamon::_Get_dynamic_specs<_Width_checker>(_Ctx.arg(static_cast<size_t>(_Specs._Dynamic_width_index)));
    }
    ...
    basic_string<_CharT> _Buffer;
    {
        ...
        if constexpr (same_as<_Ty, _CharT>) {
            if (_Specs._Type == 's' || _Specs._Type == '?') {
                hamon::_Range_formatter_format_as_string<_CharT>(
                    hamon::forward<_Range>(_Rng), _Nested_context, _Specs._Type == '?');
            } else {
                hamon::_Range_formatter_format_as_sequence(_Underlying, _Separator, _Opening_bracket, _Closing_bracket,
                    hamon::forward<_Range>(_Rng), _Nested_context);
            }
        } else {
            hamon::_Range_formatter_format_as_sequence(_Underlying, _Separator, _Opening_bracket, _Closing_bracket,
                hamon::forward<_Range>(_Rng), _Nested_context);
    ...
    const int _Width = hamon::_Measure_display_width<_CharT>(_Buffer);
    return hamon::_Write_aligned(_Ctx.out(), _Width, _Format_specs, _Fmt_align::_Left,
        [&](_FormatContext::iterator _Out) { return hamon::_Fmt_write(hamon::move(_Out), basic_string_view{_Buffer}); });
}
...
HAMON_NODISCARD constexpr const _CharT* _Parse_tuple_format_specs(
    const _CharT* _Begin, const _CharT* _End, _Callbacks_type&& _Callbacks) {
    if (_Begin == _End || *_Begin == '}') {
        return _Begin;
    }

    _Begin = hamon::_Parse_align(_Begin, _End, _Callbacks);
    if (_Begin == _End) {
        return _Begin;
    }

    _Begin = hamon::_Parse_width(_Begin, _End, _Callbacks);
    if (_Begin == _End) {
        return _Begin;
    }
...
HAMON_NODISCARD constexpr _ParseContext::iterator _Tuple_formatter_parse(tuple<formatter<_Types, _CharT>...>& _Underlying,
    basic_string_view<_CharT>& _Separator, basic_string_view<_CharT>& _Opening_bracket,
    ...
    const auto _It = hamon::_Parse_tuple_format_specs(_Ctx._Unchecked_begin(), _Ctx._Unchecked_end(), _Callback);
    if (_It != _Ctx._Unchecked_end() && *_It != '}') {
        hamon::_Throw_format_error("Missing '}' in format string.");
    }
    ...
    _Ctx.advance_to(_Ctx.begin() + (_It - _Ctx._Unchecked_begin()));
    hamon::apply([&_Ctx](auto&... _Elems) { (hamon::_Set_tuple_debug_format(_Elems, _Ctx), ...); }, _Underlying);
...
void _Tuple_formatter_format_to_context(const tuple<formatter<_Types, _CharT>...>& _Underlying,
    ...
    hamon::copy/*_Copy_unchecked*/(_Opening_bracket._Unchecked_begin(), _Opening_bracket._Unchecked_end(), _Fmt_ctx.out());
    [&]<size_t... _Indices>(index_sequence<_Indices...>) {
        auto _Single_writer = [&]<size_t _Idx>(auto& _Arg) {
            if constexpr (_Idx != 0) {
                hamon::copy/*_Copy_unchecked*/(_Separator._Unchecked_begin(), _Separator._Unchecked_end(), _Fmt_ctx.out());
            }
            hamon::get<_Idx>(_Underlying).format(_Arg, _Fmt_ctx);
        };
        (_Single_writer.template operator()<_Indices>(_Args), ...);
    }(index_sequence_for<_ArgTypes...>{});
    hamon::copy/*_Copy_unchecked*/(_Closing_bracket._Unchecked_begin(), _Closing_bracket._Unchecked_end(), _Fmt_ctx.out());
}

template <class... _Types, class _CharT, class _FormatContext, class... _ArgTypes>
HAMON_NODISCARD _FormatContext::iterator _Tuple_formatter_format(const tuple<formatter<_Types, _CharT>...>& _Underlying,
    const basic_string_view<_CharT> _Separator, const basic_string_view<_CharT> _Opening_bracket,
    ...
    if (_Specs._Dynamic_width_index >= 0) {
        _Format_specs._Width =
            hamon::_Get_dynamic_specs<_Width_checker>(_Fmt_ctx.arg(static_cast<size_t>(_Specs._Dynamic_width_index)));
    }

    if (_Format_specs._Width <= 0) {
        hamon::_Tuple_formatter_format_to_context(
    ...
    const int _Width = hamon::_Measure_display_width<_CharT>(_Tmp_str);
    return hamon::_Write_aligned(
        _Fmt_ctx.out(), _Width, _Format_specs, _Fmt_align::_Left, [&](_FormatContext::iterator _Out) {
            return hamon::_Fmt_write(hamon::move(_Out), basic_string_view<_CharT>{_Tmp_str});
        });
}

enum class _Add_newline : bool { _Nope, _Yes };

HAMON_NODISCARD inline string _Unescape_braces(const _Add_newline _Add_nl, const string_view _Old_str) {
...
template <class _CharT, class _Callbacks_type>
HAMON_NODISCARD constexpr const _CharT* _Parse_fill_align_and_width_specs(
    const _CharT* _Begin, const _CharT* _End, _Callbacks_type&& _Callbacks) {
    if (_Begin == _End || *_Begin == '}') {
        return _Begin;
    }
...
HAMON_NODISCARD constexpr _Pc::iterator _Fill_align_and_width_formatter_parse(
...
HAMON_NODISCARD _FormatContext::iterator _Fill_align_and_width_formatter_format(
    ...
    return _Write_aligned(_Format_ctx.out(), _Width, _Format_specs, _Default_align, hamon::forward<_Func>(_Fn));
}
#endif // _HAS_CXX23

}	// namespace hamon

#endif // HAMON_FORMAT_DETAIL_IMPL_HPP
