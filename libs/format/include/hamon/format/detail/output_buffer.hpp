 ...
 *
 *	@brief	output_buffer の定義
 */

#ifndef HAMON_FORMAT_DETAIL_OUTPUT_BUFFER_HPP
#define HAMON_FORMAT_DETAIL_OUTPUT_BUFFER_HPP
...
concept __fmt_char_type = hamon::same_as<_CharT, char> || hamon::same_as<_CharT, wchar_t>;

class __max_output_size
{
public:
	[[nodiscard]] explicit
	__max_output_size(size_t __max_size)
	...
	[[nodiscard]] size_t __code_units_written() const noexcept
	{
...
template <__fmt_char_type CharT>
class output_buffer
{
public:
	using value_type           = CharT;
	using __prepare_write_type = void (*)(output_buffer<CharT>&, size_t);

	[[nodiscard]] explicit
	output_buffer(CharT* __ptr, size_t __capacity, __prepare_write_type __function)
		: output_buffer {__ptr, __capacity, __function, nullptr} {}

	[[nodiscard]] explicit
	output_buffer(
		CharT* __ptr, size_t __capacity, __prepare_write_type __function, __max_output_size* __max_output_size)
		: __ptr_(__ptr), __capacity_(__capacity), __prepare_write_(__function), __max_output_size_(__max_output_size) {}

	void __buffer_flushed()
	{
		__size_ = 0;
	}

	void __buffer_moved(CharT* __ptr, size_t __capacity)
	{
		__ptr_      = __ptr;
	...
	{
		return hamon::back_insert_iterator{*this};
	}
	...
	/// conversion when CharT is wchar_t.
	...
	/// A \c fill_n wrapper.
	void __fill(size_t __n, CharT __value)
	{
		if (__max_output_size_)
		{
			__n = __max_output_size_->__write_request(__n);
...
private:
	...
	size_t __capacity_;
	size_t __size_ {0};
	void (*__prepare_write_)(output_buffer<CharT>&, size_t);
	__max_output_size* __max_output_size_;

	[[nodiscard]] size_t __available() const
	{
...
#endif // HAMON_FORMAT_DETAIL_OUTPUT_BUFFER_HPP
