...
#ifndef HAMON_FORMAT_OTHER___UNICODE_HPP
#define HAMON_FORMAT_OTHER___UNICODE_HPP

#include <hamon/format/other/__extended_grapheme_custer_property_boundary.hpp>
#include <hamon/format/__format/__indic_conjunct_break.hpp>
...
[[nodiscard]] constexpr bool __is_high_surrogate(char32_t __value)
{
	return __value >= 0xd800 && __value <= 0xdbff;
}

[[nodiscard]] constexpr bool __is_low_surrogate(char32_t __value)
{
	return __value >= 0xdc00 && __value <= 0xdfff;
}

// https://www.unicode.org/glossary/#surrogate_code_point
[[nodiscard]] inline constexpr bool __is_surrogate(char32_t __value)
{
	return __value >= 0xd800 && __value <= 0xdfff;
}

// https://www.unicode.org/glossary/#code_point
[[nodiscard]] inline constexpr bool __is_code_point(char32_t __value)
{
	return __value <= 0x10ffff;
}

// https://www.unicode.org/glossary/#unicode_scalar_value
[[nodiscard]] inline constexpr bool __is_scalar_value(char32_t __value)
{
	return __unicode::__is_code_point(__value) && !__unicode::__is_surrogate(__value);
}

template <hamon::contiguous_iterator _Iterator>
requires hamon::same_as<hamon::iter_value_t<_Iterator>, char>
constexpr bool __is_continuation(_Iterator __char, int __count)
{
	do
	{
...
{
	using _Iterator  = hamon::basic_string_view<char>::const_iterator;

public:
	constexpr explicit __code_point_view(_Iterator __first, _Iterator __last)
		: __first_(__first), __last_(__last) {}

	constexpr bool __at_end() const noexcept { return __first_ == __last_; }
	constexpr _Iterator __position() const noexcept { return __first_; }
	...
	[[nodiscard]] constexpr __consume_result __consume() noexcept
	{
		...
		switch (hamon::countl_one(static_cast<unsigned char>(*__first_)))
		{
		case 0:
...
constexpr bool __is_surrogate_pair_high(wchar_t __value)
{
	return __value >= 0xd800 && __value <= 0xdbff;
}

constexpr bool __is_surrogate_pair_low(wchar_t __value)
{
	return __value >= 0xdc00 && __value <= 0xdfff;
}
...
{
	using _Iterator  = typename hamon::basic_string_view<wchar_t>::const_iterator;
...
public:
	constexpr explicit __extended_grapheme_cluster_break(char32_t __first_code_point)
	...
	[[nodiscard]] constexpr bool operator()(char32_t __next_code_point)
	{
	...
	// evaluation cyle.
	[[nodiscard]] constexpr char32_t __current_code_point() const { return __prev_code_point_; }
...
{
	using _Iterator  = typename hamon::basic_string_view<_CharT>::const_iterator;

public:
	constexpr explicit __extended_grapheme_cluster_view(_Iterator __first, _Iterator __last)
	...
	[[nodiscard]] constexpr __cluster __consume()
	{
...
template <hamon::contiguous_iterator _Iterator>
__extended_grapheme_cluster_view(_Iterator, _Iterator)->__extended_grapheme_cluster_view<hamon::iter_value_t<_Iterator>>;

} // namespace __unicode

}	// namespace hamon

#endif // HAMON_FORMAT_OTHER___UNICODE_HPP
