...
{
	return hamon::vformat_to(hamon::move(out), fmt.str, hamon::make_format_args(args...));
}

template <typename Out, typename... Args>
Out format_to(Out out, hamon::wformat_string<Args...> fmt, Args&&... args)
{
	return hamon::vformat_to(hamon::move(out), fmt.str, hamon::make_wformat_args(args...));
}

template <typename Out, typename... Args>
Out format_to(Out out, std::locale const& loc, hamon::format_string<Args...> fmt, Args&&... args)
{
	return hamon::vformat_to(hamon::move(out), loc, fmt.str, hamon::make_format_args(args...));
}

template <typename Out, typename... Args>
Out format_to(Out out, std::locale const& loc, hamon::wformat_string<Args...> fmt, Args&&... args)
{
	return hamon::vformat_to(hamon::move(out), loc, fmt.str, hamon::make_wformat_args(args...));
}
