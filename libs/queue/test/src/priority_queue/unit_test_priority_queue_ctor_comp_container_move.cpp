/**
 *	@file	unit_test_priority_queue_ctor_comp_container_move.cpp
 *
 *	@brief	Compare と Container&& を引数に取るコンストラクタのテスト
 *
...
namespace ctor_comp_container_move_test
{
...
{
	{
		...
		static_assert( hamon::is_constructible<Queue, Compare const&, Container&&>::value, "");
		static_assert(!hamon::is_nothrow_constructible<Queue, Compare const&, Container&&>::value, "");
#if !defined(HAMON_USE_STD_PRIORITY_QUEUE)	// P0935R0
		static_assert( hamon::is_implicitly_constructible<Queue, Compare const&, Container&&>::value, "");
#endif
		static_assert(!hamon::is_trivially_constructible<Queue, Compare const&, Container&&>::value, "");

		Container c{T{3}, T{1}, T{4},};
		Queue const q(Compare{}, hamon::move(c));
		VERIFY(q.size() == 3);
		VERIFY(q.top() == T{4});
	}
	{
...
GTEST_TEST(PriorityQueueTest, CtorCompContainerMoveTest)
{
