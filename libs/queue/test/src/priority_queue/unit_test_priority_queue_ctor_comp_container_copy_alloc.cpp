/**
 *	@file	unit_test_priority_queue_ctor_comp_container_copy_alloc.cpp
 *
 *	@brief	Compare と Container const& と Alloc を引数に取るコンストラクタのテスト
 *
...
namespace ctor_comp_container_copy_alloc_test
{
...
PRIORITY_QUEUE_TEST_CONSTEXPR bool test()
{
...

	{
		...
		static_assert( hamon::is_constructible<Queue, Compare const&, Container const&, Alloc const&>::value, "");
		static_assert(!hamon::is_nothrow_constructible<Queue, Compare const&, Container const&, Alloc const&>::value, "");
		static_assert( hamon::is_implicitly_constructible<Queue, Compare const&, Container const&, Alloc const&>::value, "");
		static_assert(!hamon::is_trivially_constructible<Queue, Compare const&, Container const&, Alloc const&>::value, "");

		Alloc const a{};
		Queue const q(Compare{}, c, a);
		VERIFY(q.size() == 3);
		VERIFY(q.top() == T{4});
	}
	{
...
GTEST_TEST(PriorityQueueTest, CtorCompContainerCopyAllocTest)
{
