/**
 *	@file	unit_test_priority_queue_ctor_iterator_comp_alloc.cpp
 *
 *	@brief	イテレータと Compare と Alloc を引数に取るコンストラクタのテスト
 *
...
namespace ctor_iterator_comp_alloc_test
{
...
PRIORITY_QUEUE_TEST_CONSTEXPR bool test_impl()
{
...

	{
		...
		static_assert( hamon::is_constructible<Queue, Iterator, Iterator, Compare const&, Alloc const&>::value, "");
		static_assert(!hamon::is_nothrow_constructible<Queue, Iterator, Iterator, Compare const&, Alloc const&>::value, "");
		static_assert( hamon::is_implicitly_constructible<Queue, Iterator, Iterator, Compare const&, Alloc const&>::value, "");
		static_assert(!hamon::is_trivially_constructible<Queue, Iterator, Iterator, Compare const&, Alloc const&>::value, "");

		Compare const comp{};
		Queue const q(Iterator{v}, Iterator{v+4}, comp, a);
		VERIFY(q.size() == 4);
		VERIFY(q.top() == T{3});
	}
	{
...
GTEST_TEST(PriorityQueueTest, CtorIteratorCompAllocTest)
{
