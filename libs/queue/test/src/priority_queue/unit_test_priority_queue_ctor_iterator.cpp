/**
 *	@file	unit_test_priority_queue_ctor_iterator.cpp
 *
...
PRIORITY_QUEUE_TEST_CONSTEXPR bool test_impl()
{
...

	{
		...
		static_assert( hamon::is_constructible<Queue, Iterator, Iterator, Compare const&>::value, "");
		static_assert(!hamon::is_nothrow_constructible<Queue, Iterator, Iterator>::value, "");
		static_assert(!hamon::is_nothrow_constructible<Queue, Iterator, Iterator, Compare const&>::value, "");
		static_assert( hamon::is_implicitly_constructible<Queue, Iterator, Iterator>::value, "");
		static_assert( hamon::is_implicitly_constructible<Queue, Iterator, Iterator, Compare const&>::value, "");
		static_assert(!hamon::is_trivially_constructible<Queue, Iterator, Iterator>::value, "");
		static_assert(!hamon::is_trivially_constructible<Queue, Iterator, Iterator, Compare const&>::value, "");
		...
		{
			Queue const q(Iterator{v}, Iterator{v+4}, Compare{});
			VERIFY(q.size() == 4);
			VERIFY(q.top() == T{3});
		}
	}
...
GTEST_TEST(PriorityQueueTest, CtorIteratorTest)
{
