/**
 *	@file	unit_test_priority_queue_ctor_iterator_container_move.cpp
 *
 *	@brief	イテレータ と Container&& を引数に取るコンストラクタのテスト
 *
 *	template<class InputIterator>
 *	constexpr priority_queue(InputIterator first, InputIterator last, const Compare& x, Container&&);
 */
...
namespace ctor_iterator_container_move_test
{
...
PRIORITY_QUEUE_TEST_CONSTEXPR bool test_impl()
{
...

	{
		...
		static_assert( hamon::is_constructible<Queue, Iterator, Iterator, Compare const&, Container&&>::value, "");
		static_assert(!hamon::is_nothrow_constructible<Queue, Iterator, Iterator, Compare const&, Container&&>::value, "");
		static_assert( hamon::is_implicitly_constructible<Queue, Iterator, Iterator, Compare const&, Container&&>::value, "");
		static_assert(!hamon::is_trivially_constructible<Queue, Iterator, Iterator, Compare const&, Container&&>::value, "");

		Container c{T{1}, T{2},};
		Queue const q(Iterator{v}, Iterator{v+4}, Compare{}, hamon::move(c));
		VERIFY(q.size() == 6);
		VERIFY(q.top() == T{3});
	}
	{
...
GTEST_TEST(PriorityQueueTest, CtorIteratorContainerMoveTest)
{
