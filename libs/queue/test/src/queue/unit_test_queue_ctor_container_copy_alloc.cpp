/**
 *	@file	unit_test_queue_ctor_container_copy_alloc.cpp
 *
...
QUEUE_TEST_CONSTEXPR bool test()
{
	...
	static_assert( hamon::is_constructible<Queue, Container const&, Alloc const&>::value, "");
#if !defined(HAMON_USE_STD_QUEUE)
	static_assert(
		hamon::is_nothrow_constructible<Queue,     Container const&, Alloc const&>::value ==
		hamon::is_nothrow_constructible<Container, Container const&, Alloc const&>::value, "");
#endif
	static_assert( hamon::is_implicitly_constructible<Queue, Container const&, Alloc const&>::value, "");
	static_assert(!hamon::is_trivially_constructible<Queue, Container const&, Alloc const&>::value, "");
...
GTEST_TEST(QueueTest, CtorContainerCopyAllocTest)
{
