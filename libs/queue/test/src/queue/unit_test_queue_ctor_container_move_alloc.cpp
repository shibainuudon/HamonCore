/**
 *	@file	unit_test_queue_ctor_container_move_alloc.cpp
 *
...
QUEUE_TEST_CONSTEXPR bool test()
{
	...
	static_assert( hamon::is_constructible<Queue, Container&&, Alloc const&>::value, "");
#if !defined(HAMON_USE_STD_QUEUE)
	static_assert(
		hamon::is_nothrow_constructible<Queue,     Container&&, Alloc const&>::value ==
		hamon::is_nothrow_constructible<Container, Container&&, Alloc const&>::value, "");
#endif
	static_assert( hamon::is_implicitly_constructible<Queue, Container&&, Alloc const&>::value, "");
	static_assert(!hamon::is_trivially_constructible<Queue, Container&&, Alloc const&>::value, "");

	Container c{T{10}, T{20}, T{30}};
	Alloc a{};
	Queue const q(hamon::move(c), a);
...
GTEST_TEST(QueueTest, CtorContainerMoveAllocTest)
{
