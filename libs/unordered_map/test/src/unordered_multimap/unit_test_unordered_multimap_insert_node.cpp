/**
 *	@file	unit_test_unordered_multimap_insert_node.cpp
 *
 *	@brief	insert のテスト
 *
...
UNORDERED_MULTIMAP_TEST_CONSTEXPR bool test1()
{
	...
	static_assert(hamon::is_same<decltype(hamon::declval<Map&>().insert(hamon::declval<NodeType>())), Iterator>::value, "");
...
UNORDERED_MULTIMAP_TEST_CONSTEXPR bool test3()
{
	...
	{
		auto r = v2.insert(v1.extract(Key{3}));
		VERIFY(r == v2.end());
	}
...
GTEST_TEST(UnorderedMultimapTest, InsertNodeTest)
{
