...
private:
	...
	using HashTable  = hamon::detail::hash_table<key_type, value_type, MapHash, MapCompare, allocator_type, true>;
...
public:
	...
	unordered_multimap(
		...
		: m_allocator(a)
		, m_impl(n, hf, eql, a)	// [unord.multimap.cnstr]/1
	{}
	...
	template <HAMON_CONSTRAINT(hamon::detail::container_compatible_range, value_type, R)>
	HAMON_CXX14_CONSTEXPR
	unordered_multimap(hamon::from_range_t, R&& rg, allocator_type const& a)	// may throw
	...
	HAMON_CXX14_CONSTEXPR explicit
	unordered_multimap(Allocator const& a)	// may throw
		: unordered_multimap(size_type(), hasher(), key_equal(), a)
	{}

	HAMON_CXX14_CONSTEXPR
	unordered_multimap(unordered_multimap const& x)	// may throw
		: unordered_multimap(x.bucket_count(), x.hash_function(), x.key_eq(),
	...
	unordered_multimap(unordered_multimap&& x) HAMON_NOEXCEPT_IF(	// noexcept as an extension
	...
	HAMON_CXX14_CONSTEXPR
	unordered_multimap(unordered_multimap const& x, hamon::type_identity_t<Allocator> const& a)
		: unordered_multimap(x.bucket_count(), x.hash_function(), x.key_eq(), a)
	{
	...
	HAMON_CXX14_CONSTEXPR
	unordered_multimap(unordered_multimap&& x, hamon::type_identity_t<Allocator> const& a)
	...
	~unordered_multimap() HAMON_NOEXCEPT
	{
	...
	HAMON_CXX14_CONSTEXPR unordered_multimap&
	operator=(unordered_multimap&& x) HAMON_NOEXCEPT_IF(
		AllocTraits::is_always_equal::value &&
		hamon::is_nothrow_move_assignable<Hash>::value &&
		hamon::is_nothrow_move_assignable<Pred>::value)
	{
...
HAMON_WARNING_POP()
	...
	insert(P&& obj)
	{
		...
		return emplace(hamon::forward<P>(obj));	// may throw
	}
	...
	HAMON_CXX14_CONSTEXPR void
	swap(unordered_multimap& x) HAMON_NOEXCEPT_IF(
		AllocTraits::is_always_equal::value &&
	...
	unordered_multimap<Key, T, Hash, Pred, Alloc> const& b)
{
	...
	{
		auto eqa = a.equal_range(i->first);
		auto eqb = b.equal_range(i->first);
