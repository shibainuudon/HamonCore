/**
 *	@file	unit_test_list_append_range.cpp
 *
...
LIST_TEST_CONSTEXPR bool test_impl()
{
...

	{
		List v;
		Range rg;
		static_assert(hamon::is_same<decltype(v.append_range(rg)), void>::value, "");
		static_assert(!noexcept(v.append_range(rg)), "");
	}
	{
		List v;
		VERIFY(v.empty());
...
GTEST_TEST(ListTest, AppendRangeTest)
{
...
// 操作中に例外が発生した場合、副作用は発生しない
	...
	MayThrow::s_instance_max_count = 0;
	{
		...
		EXPECT_THROW(v.append_range(t), MayThrow::Exception);
		EXPECT_EQ(6, MayThrow::s_instance_count);
		EXPECT_EQ(3u, v.size());
		{
			auto it = v.begin();
