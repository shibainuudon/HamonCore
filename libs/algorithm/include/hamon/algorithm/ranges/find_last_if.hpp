/**
 *	@file	find_last_if.hpp
 *
 *	@brief	ranges::find_last_if の定義
 */

#ifndef HAMON_ALGORITHM_RANGES_FIND_LAST_IF_HPP
#define HAMON_ALGORITHM_RANGES_FIND_LAST_IF_HPP
...
private:
	...
	{
		Iter found = ranges::find_if(first, last, pred, proj);
		if (found == last)
		{
			return { found, found };
		}
		...
		for (;;)
		{
			first = ranges::find_if(ranges::next(first), last, pred, proj);
			if (first == last)
			{
				return { found, first };
			}
...
HAMON_INLINE_VAR HAMON_CONSTEXPR find_last_if_fn find_last_if{};
...
#endif // HAMON_ALGORITHM_RANGES_FIND_LAST_IF_HPP
