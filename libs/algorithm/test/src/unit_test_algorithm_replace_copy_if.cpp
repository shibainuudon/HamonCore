...
inline HAMON_CXX17_CONSTEXPR bool ReplaceCopyIfTest2()
{
	{
		const int a[] { 3,1,2,1,2 };
		std::array<int, 5> b{};
...
GTEST_TEST(AlgorithmTest, ReplaceCopyIfTest)
{
	...
	HAMON_CXX17_CONSTEXPR_EXPECT_TRUE(ReplaceCopyIfTest2());

	{
		const int a[] { 3,1,2,1,2 };
		std::vector<int> b;
		auto ret = hamon::replace_copy_if(hamon::begin(a), hamon::end(a), std::back_inserter(b), pred2(), 5);
		(void)ret;
		EXPECT_EQ(5u, b.size());
		EXPECT_EQ(3, b[0]);
		EXPECT_EQ(1, b[1]);
	...
	}
	{
		...
		std::list<int> b;
		auto ret = hamon::replace_copy_if(hamon::begin(a), hamon::end(a), std::back_inserter(b), [](int x) { return x == 3; }, 5);
		(void)ret;
		auto it = b.begin();
		EXPECT_EQ(5, *it++);
		EXPECT_EQ(1, *it++);
	...
	}
	{
		std::list<int> a;
		std::vector<int> b;
		auto ret = hamon::replace_copy_if(hamon::begin(a), hamon::end(a), std::back_inserter(b), [](int) { return true; }, 1);
		(void)ret;
		EXPECT_TRUE(a.empty());
		EXPECT_TRUE(b.empty());
	}
