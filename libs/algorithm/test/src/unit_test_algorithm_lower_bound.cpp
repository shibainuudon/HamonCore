...
GTEST_TEST(AlgorithmTest, LowerBoundTest)
{
	...
	}
	{
		...
		HAMON_CXX17_CONSTEXPR_OR_CONST auto ret = hamon::lower_bound(hamon::begin(a), hamon::end(a), 4);
		HAMON_CXX17_CONSTEXPR_EXPECT_TRUE(ret == hamon::next(hamon::begin(a), 2));
	}
	{
		HAMON_STATIC_CONSTEXPR std::array<int, 3> a {{ 10,20,30 }};
		HAMON_CXX17_CONSTEXPR_OR_CONST auto ret = hamon::lower_bound(hamon::begin(a), hamon::end(a), 10, pred2());
