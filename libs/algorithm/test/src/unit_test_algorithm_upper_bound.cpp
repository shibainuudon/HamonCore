...
GTEST_TEST(AlgorithmTest, UpperBoundTest)
{
	...
	}
	{
		...
		HAMON_CXX14_CONSTEXPR_OR_CONST auto ret = hamon::upper_bound(hamon::begin(a), hamon::end(a), 2);
	...
	}
	{
		...
		HAMON_CXX14_CONSTEXPR_OR_CONST auto ret = hamon::upper_bound(hamon::begin(a), hamon::end(a), 7, pred1);
		HAMON_CXX14_CONSTEXPR_EXPECT_TRUE(ret == hamon::next(hamon::begin(a), 5));
	}
	{
