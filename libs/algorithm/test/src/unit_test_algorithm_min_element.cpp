/**
 *	@file	unit_test_algorithm_min_element.cpp
 *
...
GTEST_TEST(AlgorithmTest, MinElementTest)
{
	...
	}
	{
		...
		{
			auto ret = hamon::min_element(hamon::begin(a), hamon::end(a));
			EXPECT_TRUE(ret == hamon::next(hamon::begin(a), 2));
		}
		{
			auto ret = hamon::min_element(hamon::begin(a), hamon::end(a), pred1);
			EXPECT_TRUE(ret == hamon::next(hamon::begin(a), 0));
		}
		{
			auto ret = hamon::min_element(hamon::begin(a), hamon::end(a), [](int x, int y){return x > y; });
			EXPECT_TRUE(ret == hamon::next(hamon::begin(a), 0));
		}
	}
	{
		...
		{
			auto ret = hamon::min_element(hamon::begin(a), hamon::end(a), pred2());
			EXPECT_TRUE(ret == hamon::next(hamon::begin(a), 0));
		}
		{
			auto ret = hamon::min_element(hamon::begin(a), hamon::end(a), [](int x, int y){return x > y; });
