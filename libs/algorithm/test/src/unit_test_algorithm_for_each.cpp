...
struct Sum
{
	...
	HAMON_CXX11_CONSTEXPR Sum(Sum&& rhs) noexcept : n(std::move(rhs.n)) {}
	HAMON_CXX14_CONSTEXPR Sum& operator=(Sum&& rhs) noexcept { n = std::move(rhs.n); return *this; }
...
inline HAMON_CXX17_CONSTEXPR bool ForEachStdArrayTest()
{
	{
...
GTEST_TEST(AlgorithmTest, ForEachTest)
{
	...
	HAMON_CXX17_CONSTEXPR_EXPECT_TRUE(ForEachStdArrayTest());
