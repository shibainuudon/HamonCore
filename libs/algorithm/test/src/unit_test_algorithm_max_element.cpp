...
GTEST_TEST(AlgorithmTest, MaxElementTest)
{
	...
	}
	{
		...
		{
			HAMON_CXX17_CONSTEXPR_OR_CONST auto ret = hamon::max_element(hamon::begin(a), hamon::end(a));
			HAMON_CXX17_CONSTEXPR_EXPECT_TRUE(ret == hamon::next(hamon::begin(a), 6));
		}
		{
			HAMON_CXX17_CONSTEXPR_OR_CONST auto ret = hamon::max_element(hamon::begin(a), hamon::end(a), pred1);
			HAMON_CXX17_CONSTEXPR_EXPECT_TRUE(ret == hamon::next(hamon::begin(a), 1));
		}
		{
			HAMON_CXX17_CONSTEXPR_OR_CONST auto ret = hamon::max_element(hamon::begin(a), hamon::end(a), pred2());
			HAMON_CXX17_CONSTEXPR_EXPECT_TRUE(ret == hamon::next(hamon::begin(a), 6));
		}
	}
	{
