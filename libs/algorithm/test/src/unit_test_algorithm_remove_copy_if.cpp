...
GTEST_TEST(AlgorithmTest, RemoveCopyIfTest)
{
...

	{
		const int a[] = {1,2,3,1,3,1,2};
		hamon::vector<int> b;
		auto ret = hamon::remove_copy_if(hamon::begin(a), hamon::end(a), hamon::back_inserter(b), pred1);
		(void)ret;
		EXPECT_EQ(4u, b.size());
		EXPECT_EQ(2, b[0]);
		EXPECT_EQ(3, b[1]);
	...
	}
	{
		const hamon::array<int, 7> a = {{1,2,3,1,3,1,2}};
		hamon::list<int> b;
		auto ret = hamon::remove_copy_if(hamon::begin(a), hamon::end(a), hamon::back_inserter(b), [](int x) { return x >= 3; });
		(void)ret;
		auto it = b.begin();
		EXPECT_EQ(1, *it++);
		EXPECT_EQ(2, *it++);
	...
	}
	{
		const hamon::array<int, 4> a = {{1,1,1,1}};
		hamon::vector<int> b;
		auto ret = hamon::remove_copy_if(hamon::begin(a), hamon::end(a), hamon::back_inserter(b), [](int x) { return x >= 0; });
		(void)ret;
		EXPECT_TRUE(b.empty());
	}
	{
