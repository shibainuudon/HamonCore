...
//	static_assert(!hamon::is_implicitly_constructible<matrix3x3, quaternion const&>::value, "");
	...
	{
		HAMON_CONSTEXPR auto q = quaternion::identity();
		HAMON_CONSTEXPR matrix3x3 m(q);
		HAMON_CONSTEXPR auto expected = matrix3x3::identity();

		HAMON_CXX11_CONSTEXPR_EXPECT_EQ(expected, m);
	}
	{
		HAMON_CONSTEXPR quaternion q { 1, 2, 3, 4 };
		HAMON_CONSTEXPR matrix3x3 m(q);
	...
	}
	{
		...
		const auto q = quaternion::rotation_x(angle);
		const matrix3x3 m(q);
		const auto expected = matrix3x3::rotation_x(angle);
	...
	}
	{
		...
		const auto q = quaternion::rotation_y(angle);
		const matrix3x3 m(q);
		const auto expected = matrix3x3::rotation_y(angle);
		EXPECT_NEAR((double)expected[0][0], (double)m[0][0], error);
		EXPECT_NEAR((double)expected[0][1], (double)m[0][1], error);
		EXPECT_NEAR((double)expected[0][2], (double)m[0][2], error);
		EXPECT_NEAR((double)expected[1][0], (double)m[1][0], error);
	...
	{
		const auto angle = hamon::degrees_to_radians(60);
		const auto q = quaternion::rotation_z(angle);
		const matrix3x3 m(q);
		const auto expected = matrix3x3::rotation_z(angle);
		EXPECT_NEAR((double)expected[0][0], (double)m[0][0], error);
		EXPECT_NEAR((double)expected[0][1], (double)m[0][1], error);
		EXPECT_NEAR((double)expected[0][2], (double)m[0][2], error);
		EXPECT_NEAR((double)expected[1][0], (double)m[1][0], error);
	...
	}
	{
		...
		const auto q = quaternion::rotation(axis, angle);
		const matrix3x3 m(q);
		const auto expected = matrix3x3::rotation(axis, angle);
		EXPECT_NEAR((double)expected[0][0], (double)m[0][0], error);
		EXPECT_NEAR((double)expected[0][1], (double)m[0][1], error);
		EXPECT_NEAR((double)expected[0][2], (double)m[0][2], error);
		EXPECT_NEAR((double)expected[1][0], (double)m[1][0], error);
