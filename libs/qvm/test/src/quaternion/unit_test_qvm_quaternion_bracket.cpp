...
inline HAMON_CXX14_CONSTEXPR bool BracketTest()
{
...

	{
		...
		static_assert(std::is_same<decltype(q[0]), T&>::value, "");
		static_assert(std::is_same<decltype(q[1]), T&>::value, "");
		static_assert(std::is_same<decltype(q[2]), T&>::value, "");
		static_assert(std::is_same<decltype(q[3]), T&>::value, "");
		VERIFY(q[0] == 1);
		VERIFY(q[1] == 2);
		VERIFY(q[2] == 3);
		VERIFY(q[3] == 4);
...
TYPED_TEST(QuaternionTest, BracketTest)
{
...

	{
		...
		static_assert(std::is_same<decltype(q[0]), T const&>::value, "");
		static_assert(std::is_same<decltype(q[1]), T const&>::value, "");
		static_assert(std::is_same<decltype(q[2]), T const&>::value, "");
		static_assert(std::is_same<decltype(q[3]), T const&>::value, "");
