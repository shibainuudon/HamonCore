...
TYPED_TEST(MatrixTest, IdentityTest)
{
	...
	static_assert(std::is_same<decltype(matrix3x4::identity()), matrix3x4>::value, "");
	static_assert(std::is_same<decltype(matrix4x3::identity()), matrix4x3>::value, "");
	...
	{
		HAMON_CONSTEXPR auto const m = matrix3x4::identity();
	...
	{
		HAMON_CONSTEXPR auto const m = matrix4x3::identity();
		HAMON_CXX11_CONSTEXPR_EXPECT_EQ(vector3(1, 0, 0), m[0]);
		HAMON_CXX11_CONSTEXPR_EXPECT_EQ(vector3(0, 1, 0), m[1]);
		HAMON_CXX11_CONSTEXPR_EXPECT_EQ(vector3(0, 0, 1), m[2]);
