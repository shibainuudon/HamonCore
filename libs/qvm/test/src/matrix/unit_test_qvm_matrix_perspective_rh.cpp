/**
 *	@file	unit_test_qvm_matrix_perspective_rh.cpp
 *
 *	@brief	perspective_rh のテスト
 */
...
TYPED_TEST(MatrixFloatTest, PerspectiveRHTest)
{
	using T = TypeParam;
	using matrix4x4 = hamon::qvm::matrix<T, 4, 4>;

	static_assert(hamon::is_same<
		decltype(matrix4x4::perspective_rh(T{}, T{}, T{}, T{})),
		matrix4x4
	>::value, "");

	{
		...
		HAMON_CXX11_CONSTEXPR auto const m =
			matrix4x4::perspective_rh(fovy, aspect, zn, zf);
	...
	{
		HAMON_CXX11_CONSTEXPR auto const m = matrix4x4::perspective_rh(
	...
	}
	{
		...
		HAMON_CXX11_CONSTEXPR auto const m1 =
			matrix4x4::perspective_rh(hamon::degrees_to_radians(T{45}), aspect, zn, zf);
		HAMON_CXX11_CONSTEXPR auto const m2 =
			matrix4x4::perspective_rh(hamon::units::degrees<T>{45}, aspect, zn, zf);
