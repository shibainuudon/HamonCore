...
TYPED_TEST(MatrixFloatTest, RotationXTest)
{
	...
	{
		auto const m = matrix3x3::rotation_x(hamon::degrees_to_radians(0));
		EXPECT_EQ(matrix3x3::identity(), m);
	}
	{
		auto const m = matrix3x3::rotation_x(hamon::degrees_to_radians(30));
	...
	{
		auto const m = matrix3x3::rotation_x(hamon::degrees_to_radians(90));
		HAMON_CONSTEXPR matrix3x3 const expected
		{
			1,  0, 0,
			0,  0, 1,
