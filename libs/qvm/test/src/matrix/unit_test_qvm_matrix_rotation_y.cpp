...
TYPED_TEST(MatrixFloatTest, RotationYTest)
{
	...
	static_assert(std::is_same<decltype(matrix3x3::rotation_y(0.0f)), matrix3x3>::value, "");
	static_assert(std::is_same<decltype(matrix3x3::rotation_y(0.0)),  matrix3x3>::value, "");
	static_assert(std::is_same<decltype(matrix3x3::rotation_y(0.0L)), matrix3x3>::value, "");

	{
		auto const m = matrix3x3::rotation_y(hamon::degrees_to_radians(0));
		EXPECT_EQ(matrix3x3::identity(), m);
	}
	{
		auto const m = matrix3x3::rotation_y(hamon::degrees_to_radians(45));
	...
	{
		auto const m = matrix3x3::rotation_y(hamon::degrees_to_radians(90));
		HAMON_CONSTEXPR matrix3x3 const expected
		{
			0,  0, -1,
			0,  1,  0,
