...
make_rotation_z_matrix_impl(T const& s, T const& c) HAMON_NOEXCEPT
{
	return
	{
		 c,  s,  0,
...
HAMON_NODISCARD inline HAMON_CONSTEXPR Matrix
make_rotation_z_matrix(AngleType const& angle) HAMON_NOEXCEPT
{
	using std::sin;
	using std::cos;
	using T = qvm::detail::vector_element_t<Matrix>;
	return make_rotation_z_matrix_impl<Matrix>(
