/**
 *	@file	unit_test_functional_boyer_moore_horspool_searcher.cpp
 *
 *	@brief	boyer_moore_horspool_searcher のテスト
 */
...
#if defined(HAMON_HAS_CXX17_DEDUCTION_GUIDES)
			hamon::boyer_moore_horspool_searcher    searcher{needle.begin(), needle.end()};
#else
			hamon::boyer_moore_horspool_searcher<I> searcher{needle.begin(), needle.end()};
#endif
...
#if defined(HAMON_HAS_CXX17_DEDUCTION_GUIDES)
			hamon::boyer_moore_horspool_searcher    searcher{pattern.begin(), pattern.end()};
#else
			hamon::boyer_moore_horspool_searcher<I> searcher{pattern.begin(), pattern.end()};
#endif
...
#if defined(HAMON_HAS_CXX17_DEDUCTION_GUIDES)
			hamon::boyer_moore_horspool_searcher                searcher{pattern.begin(), pattern.end(), Hash{}, icmp{}};
#else
			hamon::boyer_moore_horspool_searcher<I, Hash, icmp> searcher{pattern.begin(), pattern.end(), Hash{}, icmp{}};
#endif
			auto ret = searcher(text.begin(), text.end());
			VERIFY(ret.first  == text.end());
			VERIFY(ret.second == text.end());
