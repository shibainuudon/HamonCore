...
struct bind_back_ctor_tag{};

template <typename FD, typename... BoundArgs>
struct bind_back_impl
{
private:
...
public:
	...
	bind_back_impl(bind_back_ctor_tag, F&& f, Args&&... args)
		// [func.bind.partial]/5
		HAMON_NOEXCEPT_IF(
			hamon::conjunction<
...
template <typename F, typename... Args>
using bind_back_t = bind_back_impl<hamon::decay_t<F>, hamon::decay_t<Args>...>;

}	// namespace detail

// 22.10.14 Function templates bind_back and bind_back	[func.bind.partial]

template <typename F, typename... Args>
HAMON_CXX11_CONSTEXPR hamon::detail::bind_back_t<F, Args...>
bind_back(F&& f, Args&&... args)
	HAMON_NOEXCEPT_IF_EXPR(
		hamon::detail::bind_back_t<F, Args...>(
			hamon::detail::bind_back_ctor_tag{},
			hamon::forward<F>(f),
			hamon::forward<Args>(args)...))
{
	...
	return hamon::detail::bind_back_t<F, Args...>(
		hamon::detail::bind_back_ctor_tag{},
		hamon::forward<F>(f),
		hamon::forward<Args>(args)...);
}
...
HAMON_CXX11_CONSTEXPR hamon::detail::bind_back_t<decltype(f), Args...>
bind_back(Args&&... args)
	HAMON_NOEXCEPT_IF_EXPR(
		hamon::detail::bind_back_t<decltype(f), Args...>(
			hamon::detail::bind_back_ctor_tag{},
			f,
			hamon::forward<Args>(args)...))
{
