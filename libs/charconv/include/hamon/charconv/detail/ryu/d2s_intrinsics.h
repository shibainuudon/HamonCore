...
static inline HAMON_CXX20_CONSTEXPR uint64_t umul128_ct(const uint64_t a, const uint64_t b, uint64_t* const productHi) {
...
static inline HAMON_CXX20_CONSTEXPR uint64_t shiftright128_ct(const uint64_t lo, const uint64_t hi, const uint32_t dist) {
  if (dist == 0) return lo;
...
static inline HAMON_CXX20_CONSTEXPR uint64_t shiftright128(const uint64_t lo, const uint64_t hi, const uint32_t dist) {
...
static inline HAMON_CXX20_CONSTEXPR uint64_t umulh(const uint64_t a, const uint64_t b) {
...
static inline HAMON_CXX20_CONSTEXPR uint64_t div5(const uint64_t x) {
  return umulh(x, 0xCCCCCCCCCCCCCCCDu) >> 2;
}

static inline HAMON_CXX20_CONSTEXPR uint64_t div10(const uint64_t x) {
  return umulh(x, 0xCCCCCCCCCCCCCCCDu) >> 3;
}

static inline HAMON_CXX20_CONSTEXPR uint64_t div100(const uint64_t x) {
  return umulh(x >> 2, 0x28F5C28F5C28F5C3u) >> 2;
}

static inline HAMON_CXX20_CONSTEXPR uint64_t div1e8(const uint64_t x) {
  return umulh(x, 0xABCC77118461CEFDu) >> 26;
}

static inline HAMON_CXX20_CONSTEXPR uint64_t div1e9(const uint64_t x) {
  return umulh(x >> 9, 0x44B82FA09B5A53u) >> 11;
}

static inline HAMON_CXX20_CONSTEXPR uint32_t mod1e9(const uint64_t x) {
...
static inline HAMON_CXX20_CONSTEXPR uint32_t pow5Factor(uint64_t value) {
...
static inline HAMON_CXX20_CONSTEXPR bool multipleOfPowerOf5(const uint64_t value, const uint32_t p) {
  // I tried a case distinction on p, but there was no performance difference.
  return pow5Factor(value) >= p;
}

// Returns true if value is divisible by 2^p.
static inline HAMON_CXX20_CONSTEXPR bool multipleOfPowerOf2(const uint64_t value, const uint32_t p) {
  assert(value != 0);
  assert(p < 64);
...
static inline HAMON_CXX20_CONSTEXPR uint64_t mulShift64(const uint64_t m, const uint64_t* const mul, const int32_t j) {
  const uint128_t b0 = ((uint128_t) m) * mul[0];
  const uint128_t b2 = ((uint128_t) m) * mul[1];
  return (uint64_t) (((b0 >> 64) + b2) >> (j - 64));
}

static inline HAMON_CXX20_CONSTEXPR uint64_t mulShiftAll64(const uint64_t m, const uint64_t* const mul, const int32_t j,
...
static inline HAMON_CXX20_CONSTEXPR uint64_t mulShiftAll64(uint64_t m, const uint64_t* const mul, const int32_t j,
  uint64_t* const vp, uint64_t* const vm, const uint32_t mmShift) {
  m <<= 1;
  // m is maximum 55 bits
  uint64_t tmp;
