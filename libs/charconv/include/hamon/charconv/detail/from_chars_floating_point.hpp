...
HAMON_NODISCARD inline bool _Shift_left(_Big_integer_flt& _Xval, const uint32_t _Nx) noexcept {
    if (_Xval._Myused == 0) {
        return true;
    }
...
HAMON_NODISCARD inline uint32_t _Add_carry(uint32_t& _Ux1, const uint32_t _Ux2, const uint32_t _U_carry) noexcept {
...
HAMON_NODISCARD inline uint32_t _Add_multiply_carry(
...
HAMON_NODISCARD inline uint32_t _Multiply_core(
...
HAMON_NODISCARD inline bool _Multiply(_Big_integer_flt& _Multiplicand, const uint32_t _Multiplier) noexcept {
    if (_Multiplier == 0) {
...
HAMON_NODISCARD inline bool _Multiply(_Big_integer_flt& _Multiplicand, const _Big_integer_flt& _Multiplier) noexcept {
    if (_Multiplicand._Myused == 0) {
        return true;
    }
...
// false if it overflowed. When overflow occurs, the high-precision integer is reset to zero.
HAMON_NODISCARD inline bool _Multiply_by_power_of_ten(_Big_integer_flt& _Xval, const uint32_t _Power) noexcept {
...
//HAMON_NODISCARD inline bool _Should_round_up(
...
template <class _FloatingType>
HAMON_NODISCARD errc _Assemble_floating_point_value(const uint64_t _Initial_mantissa, const int32_t _Initial_exponent,
...
template <class _FloatingType>
HAMON_NODISCARD errc _Assemble_floating_point_value_from_big_integer_flt(const _Big_integer_flt& _Integer_value,
...
inline void _Accumulate_decimal_digits_into_big_integer_flt(
    ...
    for (const uint8_t* _It = _First_digit; _It != _Last_digit; ++_It) {
        if (_Accumulator_count == 9) {
            /*[[maybe_unused]]*/ const bool _Success1 = _Multiply(_Result, 1000000000); // assumes no overflow
			(void)_Success1;
...
template <class _FloatingType>
HAMON_NODISCARD errc _Convert_decimal_string_to_floating_type(
    ...
    const uint32_t _Positive_exponent      = static_cast<uint32_t>((hamon::max) (0, _Data._Myexponent));
    const uint32_t _Integer_digits_present = (hamon::min) (_Positive_exponent, _Data._Mymantissa_count);
...
template <class _FloatingType>
HAMON_NODISCARD errc _Convert_hexadecimal_string_to_floating_type(
    const _Floating_point_string& _Data, _FloatingType& _Result, bool _Has_zero_tail) noexcept {
    using _Traits = _Floating_type_traits<_FloatingType>;
...
HAMON_NODISCARD hamon::from_chars_result _Ordinary_floating_from_chars(const char* const first, const char* const last,
    ...
    if (_Exponent > 0 && _Exponent_adjustment < 0) {
        if (_Is_hexadecimal) {
            const ptrdiff_t _Further_adjustment = (hamon::max) (-((_Exponent - 1) / 4 + 1), _Exponent_adjustment);
            _Exponent += _Further_adjustment * 4;
            _Exponent_adjustment -= _Further_adjustment;
        } else {
            const ptrdiff_t _Further_adjustment = (hamon::max) (-_Exponent, _Exponent_adjustment);
            _Exponent += _Further_adjustment;
            _Exponent_adjustment -= _Further_adjustment;
        }
    } else if (_Exponent < 0 && _Exponent_adjustment > 0) {
        if (_Is_hexadecimal) {
            const ptrdiff_t _Further_adjustment = (hamon::min) ((-_Exponent - 1) / 4 + 1, _Exponent_adjustment);
            _Exponent += _Further_adjustment * 4;
            _Exponent_adjustment -= _Further_adjustment;
        } else {
            const ptrdiff_t _Further_adjustment = (hamon::min) (-_Exponent, _Exponent_adjustment);
            _Exponent += _Further_adjustment;
            _Exponent_adjustment -= _Further_adjustment;
        }
    }
