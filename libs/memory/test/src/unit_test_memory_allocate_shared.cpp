/**
 *	@file	unit_test_memory_allocate_shared.cpp
 *
 *	@brief	allocate_shared のテスト
 *
 ...
 *
 *	template<class T, class A>
 *	shared_ptr<T> allocate_shared(const A& a, size_t N, const remove_extent_t<T>& u);
 *
 *	template<class T, class A>
...
bool test()
{
	...
	// (1)
	{
		auto sp = hamon::allocate_shared<int>(alloc, 2);
	...
	{
		auto sp = hamon::allocate_shared<X>(alloc, 10, 20, 30);
	...
	// (2)
	{
		auto sp = hamon::allocate_shared<int[]>(alloc, 3);
		static_assert(hamon::is_same<decltype(sp), hamon::shared_ptr<int[]>>::value, "");
		VERIFY(sp.get() != nullptr);
		VERIFY(sp.use_count() == 1);
		VERIFY(sp[0] == 0);
	...
	{
		auto sp = hamon::allocate_shared<long[][3]>(alloc, 4);
	...
	{
		auto sp = hamon::allocate_shared<int[][2][3]>(alloc, 4);
	...
	// (3)
	{
		auto sp = hamon::allocate_shared<long[4]>(alloc);
	...
	{
		auto sp = hamon::allocate_shared<float[3][2]>(alloc);
	...
	{
		auto sp = hamon::allocate_shared<int[2][3][4]>(alloc);
	...
	// (4)
	{
		auto sp = hamon::allocate_shared<int[]>(alloc, 3, 42);
		static_assert(hamon::is_same<decltype(sp), hamon::shared_ptr<int[]>>::value, "");
		VERIFY(sp.get() != nullptr);
		VERIFY(sp.use_count() == 1);
		VERIFY(sp[0] == 42);
	...
	{
		auto sp = hamon::allocate_shared<double[][2]>(alloc, 4, {3.0, 4.0});
	...
	}
	{
		...
		auto sp = hamon::allocate_shared<int[][2][3]>(alloc, 4, a);
		static_assert(hamon::is_same<decltype(sp), hamon::shared_ptr<int[][2][3]>>::value, "");
		VERIFY(sp.get() != nullptr);
		VERIFY(sp.use_count() == 1);
		VERIFY(sp[0][0][0] == 1);
	...
	// (5)
	{
		auto sp = hamon::allocate_shared<float[3]>(alloc, 5);
	...
	{
		auto sp = hamon::allocate_shared<double[3][2]>(alloc, {3.0, 4.0});
	...
	}
	{
		...
		auto sp = hamon::allocate_shared<double[4][3][2]>(alloc, a);
...
GTEST_TEST(MemoryTest, AllocateSharedTest)
{
