/**
 *	@file	unit_test_memory_uninitialized_move_n.cpp
 *
 *	@brief	uninitialized_move_n のテスト
 */
...
namespace uninitialized_move_n_test
{
...
GTEST_TEST(MemoryTest, UninitializedMoveNTest)
{
	{
		Y a[100];
		Y b[100];

		auto ret = hamon::uninitialized_move_n(a, 10, b);
		...
		EXPECT_TRUE(ret.first  == a + 10);
		EXPECT_TRUE(ret.second == b + 10);
	}
	{
		std::list<int> a = {10, 11, 12, 13};
		int b[10];

		auto ret = hamon::uninitialized_move_n(a.begin(), 4, b);
		...
		EXPECT_TRUE(ret.first  == a.end());
		EXPECT_TRUE(ret.second == b + 4);
	}
	{
		Z v[] = { 1, 2, 3 };
		...
		auto ret = hamon::uninitialized_move_n(v, size, p);
		EXPECT_TRUE(ret.first  == v + size);
		EXPECT_TRUE(ret.second == p + size);
...
#if !defined(HAMON_NO_EXCEPTIONS)
	{
		...
		hamon::uninitialized_move_n(v.begin(), size, p);

		EXPECT_EQ(7, ThrowOnMove::count);
		EXPECT_EQ(3, ThrowOnMove::move_count);

		EXPECT_ANY_THROW(hamon::uninitialized_move_n(v.begin(), size, p));
