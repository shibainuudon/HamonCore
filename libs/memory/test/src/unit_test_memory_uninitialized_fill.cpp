/**
 *	@file	unit_test_memory_uninitialized_fill.cpp
 *
 *	@brief	uninitialized_fill のテスト
 */
...
namespace uninitialized_fill_test
{
...
GTEST_TEST(MemoryTest, UninitializedFillTest)
{
	{
		int a[32];
		hamon::uninitialized_fill(a, a + 4, 42);
		EXPECT_EQ(42, a[0]);
		EXPECT_EQ(42, a[1]);
		EXPECT_EQ(42, a[2]);
		EXPECT_EQ(42, a[3]);
	}
	{
		...
		hamon::uninitialized_fill(b, b + 10, X{});
	...
	}
	{
		...
		hamon::uninitialized_fill(p, p + size, 42);
...
#if !defined(HAMON_NO_EXCEPTIONS)
	{
		...
		hamon::uninitialized_fill(p, p + size, x);

		EXPECT_EQ(4, ThrowOnCopy::count);
		EXPECT_EQ(3, ThrowOnCopy::copy_count);

		EXPECT_ANY_THROW(hamon::uninitialized_fill(p, p + size, x));
