/**
 *	@file	unit_test_ratio_ratio_equal.cpp
 *
 *	@brief	ratio_equal のテスト
 */
...
#define HAMON_RATIO_EQUAL_TEST(b, R1, R2)	\
	static_assert(hamon::ratio_equal_v<R1, R2>      == b, "");	\
	static_assert(hamon::ratio_equal<R1, R2>::value == b, "");	\
	static_assert(hamon::ratio_equal<R1, R2>{}()    == b, "");	\
	static_assert(hamon::ratio_equal<R1, R2>{}      == b, "")
...
GTEST_TEST(RatioTest, RatioEqualTest)
{
	{
		using r1 = hamon::ratio<2, 5>;
		using r2 = hamon::ratio<2, 5>;
		HAMON_RATIO_EQUAL_TEST(true, r1, r2);
	}
	{
		using r1 = hamon::ratio<1, 5>;
		using r2 = hamon::ratio<2, 5>;
		HAMON_RATIO_EQUAL_TEST(false, r1, r2);
	}
	{
		using r1 = hamon::ratio<2, 5>;
