/**
 *	@file	unit_test_ratio_ratio_less_equal.cpp
 *
 *	@brief	ratio_less_equal のテスト
 */
...
#define HAMON_RATIO_LESS_EQUAL_TEST(b, R1, R2)	\
	static_assert(hamon::ratio_less_equal_v<R1, R2>      == b, "");	\
	static_assert(hamon::ratio_less_equal<R1, R2>::value == b, "");	\
	static_assert(hamon::ratio_less_equal<R1, R2>{}()    == b, "");	\
	static_assert(hamon::ratio_less_equal<R1, R2>{}      == b, "")
...
GTEST_TEST(RatioTest, RatioLessEqualTest)
{
	{
