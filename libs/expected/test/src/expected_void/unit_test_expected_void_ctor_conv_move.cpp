/**
 *	@file	unit_test_expected_void_ctor_conv_move.cpp
 *
...
// test explicit
static_assert(hamon::is_convertible<hamon::expected<void, int>&&, hamon::expected<void, long>>::value, "");

// !is_convertible_v<GF, E>.
static_assert(hamon::is_constructible<hamon::expected<void, CtorFrom<int>>, hamon::expected<void, int>&&>::value, "");
static_assert(!hamon::is_convertible<hamon::expected<void, int>&&, hamon::expected<void, CtorFrom<int>>>::value, "");
...
{
	explicit NothrowCtorFrom(T const&);
	explicit NothrowCtorFrom(T&&) noexcept;
};

static_assert(!hamon::is_nothrow_constructible<hamon::expected<void, CtorFrom<int>>, hamon::expected<void, int>&&>::value, "");
static_assert( hamon::is_nothrow_constructible<hamon::expected<void, NothrowCtorFrom<int>>, hamon::expected<void, int>&&>::value, "");
#endif
...
HAMON_CXX14_CONSTEXPR bool test()
{
	...
	{
		hamon::expected<Void, int> src{hamon::in_place};
		hamon::expected<Void, double> dst = hamon::move(src);
		VERIFY(src.has_value());
		VERIFY(dst.has_value());
	}
	{
		hamon::expected<Void, int> src{hamon::unexpect, 5};
		hamon::expected<Void, double> dst = hamon::move(src);
		VERIFY(!src.has_value());
		VERIFY(!dst.has_value());
//		VERIFY(src.error() == 5);
	...
	{
		hamon::expected<Void, int> src{hamon::unexpect, 10};
...
HAMON_CXX14_CONSTEXPR bool test2()
{
...

	{
		...
		hamon::expected<Void, IntTuple> dst = hamon::move(src);
		VERIFY(!src.has_value());
		VERIFY(!dst.has_value());
...
GTEST_TEST(ExpectedVoidTest, CtorConvMoveTest)
{
