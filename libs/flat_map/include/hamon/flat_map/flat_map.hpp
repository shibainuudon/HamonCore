...
public:
	...
	HAMON_CXX14_CONSTEXPR flat_map&
	operator=(flat_map const&) = default;
	...
	{
		auto gd1 = hamon::detail::make_scope_guard([&]() noexcept { other.clear(); });
		{
			auto gd2 = hamon::detail::make_exception_guard([&]() noexcept { clear(); });
			this->c       = hamon::move(other.c);
	...
	{
		return this->insert_or_assign_hint_impl(hint, k, hamon::forward<M>(obj)).first;
	}
	...
	{
		return this->insert_or_assign_hint_impl(hint, hamon::move(k), hamon::forward<M>(obj)).first;
	}
	...
	{
		return this->insert_or_assign_hint_impl(hint, hamon::forward<K>(k), hamon::forward<M>(obj)).first;
	}
