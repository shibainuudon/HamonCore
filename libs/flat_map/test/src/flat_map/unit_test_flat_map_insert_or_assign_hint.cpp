...
void test_exceptions()
{
	...
	}
	{
		...
		Key const k{0};
		EXPECT_THROW(v.insert_or_assign(v.end(), k, 0), int);
		EXPECT_TRUE(check_invariant(v));
	}
	{
		using KeyContainer = TKeyContainer<Key>;
		...
		EXPECT_THROW(v.insert_or_assign(v.end(), 0, 0), int);
		EXPECT_TRUE(check_invariant(v));
	}
	{
		...
		EXPECT_THROW(v.insert_or_assign(v.end(), 0, 0), std::bad_alloc);
		EXPECT_TRUE(check_invariant(v));
	}
	{
		using KeyContainer = TKeyContainer<Key, ThrowAllocator<Key>>;
		...
		Key const k{0};
		EXPECT_THROW(v.insert_or_assign(v.end(), k, 0), std::bad_alloc);
		EXPECT_TRUE(check_invariant(v));
	}
#endif
}
