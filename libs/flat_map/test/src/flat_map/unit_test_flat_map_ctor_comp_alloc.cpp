/**
 *	@file	unit_test_flat_map_ctor_comp_alloc.cpp
 *
...
FLAT_MAP_TEST_CONSTEXPR bool test()
{
	...
	}
	{
		...
		Alloc const alloc;
		hamon::flat_map<Key, T, TestLess<Key>, KeyContainer, MappedContainer> v{comp, alloc};
		VERIFY(check_invariant(v));
		VERIFY(v.empty());
		VERIFY(v.size() == 0);
		VERIFY(v.begin() == v.end());
...
GTEST_TEST(FlatMapTest, CtorCompAllocTest)
{
