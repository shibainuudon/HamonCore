/**
 *	@file	unit_test_flat_map_insert_init_list.cpp
 *
 *	@brief	insert のテスト
 *
...
HAMON_CXX20_CONSTEXPR bool test()
{
	...
	static_assert(hamon::is_same<decltype(hamon::declval<Map&>().insert(hamon::sorted_unique, hamon::declval<hamon::initializer_list<ValueType>>())), void>::value, "");
	static_assert(!noexcept(hamon::declval<Map&>().insert(hamon::declval<hamon::initializer_list<ValueType>>())), "");
	static_assert(!noexcept(hamon::declval<Map&>().insert(hamon::sorted_unique, hamon::declval<hamon::initializer_list<ValueType>>())), "");
...
void test_exceptions()
{
	...
	}
	{
		...
		EXPECT_THROW(v.insert(hamon::sorted_unique, {
			{Key{11}, T{10}},
			{Key{12}, T{20}},
			{Key{13}, T{30}},
			{Key{15}, T{40}},
...
GTEST_TEST(FlatMapTest, InsertInitListTest)
{
