/**
 *	@file	unit_test_flat_map_ctor_first_last_alloc.cpp
 *
...
FLAT_MAP_TEST_CONSTEXPR bool test_impl()
{
	...
	static_assert( hamon::is_constructible<Map, hamon::sorted_unique_t, InputIterator, InputIterator, Alloc const&>::value, "");
	static_assert( hamon::is_constructible<Map, hamon::sorted_unique_t, InputIterator, InputIterator, Compare const&, Alloc const&>::value, "");
	...
	static_assert(!hamon::is_nothrow_constructible<Map, hamon::sorted_unique_t, InputIterator, InputIterator, Alloc const&>::value, "");
	static_assert(!hamon::is_nothrow_constructible<Map, hamon::sorted_unique_t, InputIterator, InputIterator, Compare const&, Alloc const&>::value, "");
	...
	static_assert( hamon::is_implicitly_constructible<Map, hamon::sorted_unique_t, InputIterator, InputIterator, Alloc const&>::value, "");
	static_assert( hamon::is_implicitly_constructible<Map, hamon::sorted_unique_t, InputIterator, InputIterator, Compare const&, Alloc const&>::value, "");
	...
	static_assert(!hamon::is_trivially_constructible<Map, hamon::sorted_unique_t, InputIterator, InputIterator, Alloc const&>::value, "");
	static_assert(!hamon::is_trivially_constructible<Map, hamon::sorted_unique_t, InputIterator, InputIterator, Compare const&, Alloc const&>::value, "");
	...
	}
	{
		...
		Alloc const alloc;
		hamon::flat_map<Key, T, TestLess<Key>, KeyContainer, MappedContainer> v{InputIterator{a}, InputIterator{a + 3}, comp, alloc};
		VERIFY(check_invariant(v));
		VERIFY(v.size() == 3);
	...
	}
	{
		...
		Alloc const alloc;
		Map v{hamon::sorted_unique, InputIterator{a}, InputIterator{a + 3}, alloc};
		VERIFY(check_invariant(v));
		VERIFY(!v.empty());
		VERIFY(v.size() == 3);
		VERIFY(v[Key{1}] == T{10});
	...
	}
	{
		...
		Alloc const alloc;
		hamon::flat_map<Key, T, TestLess<Key>, KeyContainer, MappedContainer> v{hamon::sorted_unique, InputIterator{a}, InputIterator{a + 4}, comp, alloc};
		VERIFY(check_invariant(v));
		VERIFY(!v.empty());
		VERIFY(v.size() == 4);
		VERIFY(v.key_comp() == comp);
...
GTEST_TEST(FlatMapTest, CtorFirstLastAllocTest)
{
