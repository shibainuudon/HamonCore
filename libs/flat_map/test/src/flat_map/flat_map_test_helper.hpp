...
check_invariant(FlatMap const& m)
{
	...
	using KeyCompare = typename FlatMap::key_compare;
	using KeyType = typename FlatMap::key_type;
	...
	if (!hamon::is_unique(m.keys().begin(), m.keys().end(), KeyEqual(m.key_comp())))
	{
		return false;
	}
