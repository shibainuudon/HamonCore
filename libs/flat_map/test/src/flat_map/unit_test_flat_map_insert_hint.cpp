...
void test_exceptions()
{
	...
	}
	{
		...
		EXPECT_THROW(v.insert(v.begin(), ValueType(0, 0)), int);
		EXPECT_TRUE(check_invariant(v));
	}
	{
...
GTEST_TEST(FlatMapTest, InsertHintTest)
{
	...
	HAMON_CXX20_CONSTEXPR_EXPECT_TRUE((test<hamon::vector<float>, hamon::deque<char>, hamon::less<float>>()));
	HAMON_CXX20_CONSTEXPR_EXPECT_TRUE((test<hamon::deque<char>, hamon::vector<long>, hamon::greater<char>>()));
