/**
 *	@file	unit_test_flat_map_ctor_range.cpp
 *
...
FLAT_MAP_TEST_CONSTEXPR bool test_impl()
{
	...
	}
	{
		...
		hamon::flat_map<Key, T, TestLess<Key>, KeyContainer, MappedContainer> v{hamon::from_range, r, comp};
		VERIFY(check_invariant(v));
		VERIFY(!v.empty());
		VERIFY(v.size() == 3);
		auto it = v.begin();
...
GTEST_TEST(FlatMapTest, CtorRangeTest)
{
