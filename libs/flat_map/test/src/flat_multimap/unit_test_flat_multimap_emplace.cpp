/**
 *	@file	unit_test_flat_multimap_emplace.cpp
 *
 *	@brief	emplace のテスト
 *
...
FLAT_MAP_TEST_CONSTEXPR bool test()
{
	...
	static_assert(hamon::is_same<decltype(hamon::declval<Map&>().emplace()), Iterator>::value, "");
...
void test_exceptions()
{
	...
	using Comp = hamon::less<Key>;
	{
		...
		EXPECT_THROW(v.emplace(ValueType(1, 1)), std::bad_alloc);
	...
	}
	{
		...
		EXPECT_THROW(v.emplace(ValueType(3, 3)), int);
		EXPECT_TRUE(check_invariant(v));
	}
	{
		using KeyContainer = TKeyContainer<Key>;
		...
		EXPECT_THROW(v.emplace(ValueType(1, 5)), int);
		EXPECT_TRUE(check_invariant(v));
	}
	{
