/**
 *	@file	unit_test_flat_multimap_insert_hint_heterogeneous.cpp
 *
 *	@brief	insert のテスト
 *
 *	template<class P>
...
FLAT_MAP_TEST_CONSTEXPR bool test()
{
	...
	static_assert(!noexcept(hamon::declval<Map&>().insert(hamon::declval<ConstIterator>(), hamon::declval<std::pair<double, long>>())), "");
	{
		Map v;
		{
			auto r = v.insert(v.cbegin(), std::make_pair(1, 10.0f));
	...
	static_assert(!noexcept(hamon::declval<Map&>().insert(hamon::declval<ConstIterator>(), hamon::declval<std::tuple<double, long>>())), "");
	{
		Map v;
		{
			auto r = v.insert(v.end(), std::make_tuple(1, 10.0f));
			VERIFY(r->first == Key{1});
			VERIFY(r->second == T{10});
		}
		VERIFY(check_invariant(v));
		...
		VERIFY(v.count(Key{3}) == 0);
		{
			auto r = v.insert(v.cend(), std::make_tuple(short{2}, 30.0f));
			VERIFY(r->first == Key{2});
			VERIFY(r->second == T{30});
		}
		VERIFY(check_invariant(v));
	...
	static_assert(!noexcept(hamon::declval<Map&>().insert(hamon::declval<ConstIterator>(), hamon::declval<std::array<double, 2>>())), "");
	{
		Map v;
		{
			auto r = v.insert(v.begin(), std::array<int, 2>{2, 10});
			VERIFY(r->first == Key{2});
			VERIFY(r->second == T{10});
		}
		VERIFY(check_invariant(v));
		...
		VERIFY(v.count(Key{3}) == 0);
		{
			auto r = v.insert(v.cbegin(), std::array<short, 2>{1, 30});
			VERIFY(r->first == Key{1});
			VERIFY(r->second == T{30});
		}
		VERIFY(check_invariant(v));
...
void test_exceptions()
{
	...
	using Comp = hamon::less<Key>;
	{
		...
		EXPECT_THROW(v.insert(v.end(), std::make_pair(1, 1)), std::bad_alloc);
	...
	}
	{
		...
		EXPECT_THROW(v.insert(v.end(), std::array<int, 2>{3, 3}), int);
		EXPECT_TRUE(check_invariant(v));
	}
	{
		using KeyContainer = TKeyContainer<Key>;
		...
		EXPECT_THROW(v.insert(v.end(), std::make_pair(1, 5)), int);
		EXPECT_TRUE(check_invariant(v));
	}
	{
...
GTEST_TEST(FlatMultimapTest, InsertHintHeterogeneousTest)
{
