/**
 *	@file	unit_test_flat_multimap_ctor_range_alloc.cpp
 *
...
FLAT_MAP_TEST_CONSTEXPR bool test_impl()
{
	...
	}
	{
		...
		Alloc const alloc;
		hamon::flat_multimap<Key, T, TestLess<Key>, KeyContainer, MappedContainer> v{hamon::from_range, r, comp, alloc};
		VERIFY(check_invariant(v));
		VERIFY(!v.empty());
		VERIFY(v.size() == 3);
		auto it = v.begin();
...
GTEST_TEST(FlatMultimapTest, CtorRangeAllocTest)
{
