/**
 *	@file	unit_test_flat_multimap_upper_bound.cpp
 *
...
FLAT_MAP_TEST_CONSTEXPR bool test()
{
...

	{
		...
		VERIFY(v.upper_bound(Key{2}) == (v.begin() + 4));
		...
		VERIFY(cv.upper_bound(Key{2}) == (cv.begin() + 4));
		VERIFY(cv.upper_bound(Key{3}) == cv.end());
		VERIFY(cv.upper_bound(Key{4}) == cv.end());
	}
	{
...
GTEST_TEST(FlatMultimapTest, UpperBoundTest)
{
