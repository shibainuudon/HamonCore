/**
 *	@file	unit_test_flat_multimap_insert_first_last.cpp
 *
 *	@brief	insert のテスト
 *
...
FLAT_MAP_TEST_CONSTEXPR bool test_impl()
{
	...
	static_assert(hamon::is_same<decltype(hamon::declval<Map&>().insert(hamon::sorted_equivalent, hamon::declval<InputIterator>(), hamon::declval<InputIterator>())), void>::value, "");
	static_assert(!noexcept(hamon::declval<Map&>().insert(hamon::declval<InputIterator>(), hamon::declval<InputIterator>())), "");
	static_assert(!noexcept(hamon::declval<Map&>().insert(hamon::sorted_equivalent, hamon::declval<InputIterator>(), hamon::declval<InputIterator>())), "");
	...
	}
	{
		Map v;
		{
			...
			v.insert(hamon::sorted_equivalent, InputIterator{a}, InputIterator{a + 3});
		}
		VERIFY(check_invariant(v));
		VERIFY(v.size() == 3);
		VERIFY(v.count(Key{1}) == 1);
		...
		VERIFY(v.count(Key{5}) == 0);
		{
			...
			hamon::ranges::sort(a, v.value_comp());
			v.insert(hamon::sorted_equivalent, InputIterator{a}, InputIterator{a + 4});
		}
		VERIFY(check_invariant(v));
		VERIFY(v.size() == 7);
		VERIFY(v.count(Key{1}) == 2);
...
void test_exceptions()
{
	...
	}
	{
		...
		EXPECT_THROW(v.insert(hamon::sorted_equivalent, hamon::ranges::begin(x), hamon::ranges::end(x)), std::bad_alloc);
		EXPECT_TRUE(check_invariant(v));
	}
	{
...
GTEST_TEST(FlatMultimapTest, InsertFirstLastTest)
{
