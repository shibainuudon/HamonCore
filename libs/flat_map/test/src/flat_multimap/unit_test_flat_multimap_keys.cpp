/**
 *	@file	unit_test_flat_multimap_keys.cpp
 *
 *	@brief	keys のテスト
 *
...
FLAT_MAP_TEST_CONSTEXPR bool test()
{
...

	{
		...
		auto const& x = v.keys();
		VERIFY(x.size() == 4);
		Key const expected[] = {Key{1}, Key{1}, Key{2}, Key{3}};
	...
	}
	{
		...
		auto const& x = v.keys();
		VERIFY(x.size() == 5);
		Key const expected[] = {Key{4}, Key{3}, Key{2}, Key{2}, Key{1}};
		VERIFY(hamon::ranges::equal(x, expected));
	}
...
GTEST_TEST(FlatMultimapTest, KeysTest)
{
