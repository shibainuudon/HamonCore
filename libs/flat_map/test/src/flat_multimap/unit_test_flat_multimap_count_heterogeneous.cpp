/**
 *	@file	unit_test_flat_multimap_count_heterogeneous.cpp
 *
 *	@brief	count のテスト
 *
...
GTEST_TEST(FlatMultimapTest, CountHeterogeneousTest)
{
...

	{
		...
		EXPECT_TRUE(fm.count((const char*)"Alice") == 2);
		EXPECT_TRUE(fm.count((const char*)"Bob")   == 1);
		EXPECT_TRUE(fm.count((const char*)"Eve")   == 0);
	}
}
