/**
 *	@file	unit_test_deque_assign_range.cpp
 *
...
HAMON_CXX20_CONSTEXPR bool test()
{
	...
	static_assert(hamon::is_same<
		decltype(hamon::declval<Deque>().assign_range(hamon::declval<Range&&>())),
		void
	>::value, "");
	static_assert(hamon::is_same<
		decltype(hamon::declval<Deque>().assign_range(hamon::declval<Range const&>())),
		void
	>::value, "");
	static_assert(!noexcept(
		hamon::declval<Deque>().assign_range(hamon::declval<Range&&>())), "");
	static_assert(!noexcept(
		hamon::declval<Deque>().assign_range(hamon::declval<Range const&>())), "");
...
GTEST_TEST(DequeTest, AssignRangeTest)
{
