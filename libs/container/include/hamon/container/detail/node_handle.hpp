...
struct set_node_handle_base
{
	...
	{
		return static_cast<Derived const*>(this)->ptr()->value();
	}
};
...
struct map_node_handle_base
{
	...
	{
		return const_cast<key_type&>(static_cast<Derived const*>(this)->ptr()->value().first);
	}

	HAMON_CXX11_CONSTEXPR mapped_type& mapped() const
	{
		return static_cast<Derived const*>(this)->ptr()->value().second;
	}
};
