/**
 *	@file	unit_test_stack_ctor_copy_alloc.cpp
 *
 *	@brief	const stack& と Alloc を引数に取るコンストラクタのテスト
 *
...
STACK_TEST_CONSTEXPR bool test()
{
	...
	static_assert( hamon::is_constructible<Stack, Stack const&, Alloc const&>::value, "");
#if !defined(HAMON_USE_STD_STACK)
	static_assert(
		hamon::is_nothrow_constructible<Stack,     Stack const&,     Alloc const&>::value ==
		hamon::is_nothrow_constructible<Container, Container const&, Alloc const&>::value, "");
#endif
	static_assert( hamon::is_implicitly_constructible<Stack, Stack const&, Alloc const&>::value, "");
	static_assert(!hamon::is_trivially_constructible<Stack, Stack const&, Alloc const&>::value, "");

	Container const c{T{30}, T{10}, T{40}};
	Stack const s1(c);
...
GTEST_TEST(StackTest, CtorCopyAllocTest)
{
