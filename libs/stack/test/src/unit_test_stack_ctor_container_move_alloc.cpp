/**
 *	@file	unit_test_stack_ctor_container_move_alloc.cpp
 *
 *	@brief	Container&& と Alloc を引数に取るコンストラクタのテスト
 *
...
namespace ctor_container_move_alloc_test
{
...
STACK_TEST_CONSTEXPR bool test()
{
	...
	static_assert( hamon::is_constructible<Stack, Container&&, Alloc const&>::value, "");
#if !defined(HAMON_USE_STD_STACK)
	static_assert(
		hamon::is_nothrow_constructible<Stack,     Container&&, Alloc const&>::value ==
		hamon::is_nothrow_constructible<Container, Container&&, Alloc const&>::value, "");
#endif
	static_assert( hamon::is_implicitly_constructible<Stack, Container&&, Alloc const&>::value, "");
	static_assert(!hamon::is_trivially_constructible<Stack, Container&&, Alloc const&>::value, "");

	Container c{T{3}, T{2}, T{1}};
	Alloc a{};
	Stack const s(hamon::move(c), a);
...
GTEST_TEST(StackTest, CtorContainerMoveAllocTest)
{
