/**
 *	@file	unit_test_stack_ctor_container_copy_alloc.cpp
 *
 *	@brief	const Container& と Alloc を引数に取るコンストラクタのテスト
 *
...
namespace ctor_container_copy_alloc_test
{
...
STACK_TEST_CONSTEXPR bool test()
{
	...
	static_assert( hamon::is_constructible<Stack, Container const&, Alloc const&>::value, "");
#if !defined(HAMON_USE_STD_STACK)
	static_assert(
		hamon::is_nothrow_constructible<Stack,     Container const&, Alloc const&>::value ==
		hamon::is_nothrow_constructible<Container, Container const&, Alloc const&>::value, "");
#endif
	static_assert( hamon::is_implicitly_constructible<Stack, Container const&, Alloc const&>::value, "");
	static_assert(!hamon::is_trivially_constructible<Stack, Container const&, Alloc const&>::value, "");

	Container const c{T{3}, T{2}, T{1}};
	Alloc a{};
	Stack const s(c, a);
...
GTEST_TEST(StackTest, CtorContainerCopyAllocTest)
{
