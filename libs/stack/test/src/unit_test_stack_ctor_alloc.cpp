/**
 *	@file	unit_test_stack_ctor_alloc.cpp
 *
...
STACK_TEST_CONSTEXPR bool test()
{
	...
	static_assert( hamon::is_constructible<Stack, Alloc const&>::value, "");
#if !defined(HAMON_USE_STD_STACK)
	static_assert(
		hamon::is_nothrow_constructible<Stack,     Alloc const&>::value ==
		hamon::is_nothrow_constructible<Container, Alloc const&>::value, "");
#endif
	static_assert(!hamon::is_implicitly_constructible<Stack, Alloc const&>::value, "");
	static_assert(!hamon::is_trivially_constructible<Stack, Alloc const&>::value, "");
...
GTEST_TEST(StackTest, CtorAllocTest)
{
