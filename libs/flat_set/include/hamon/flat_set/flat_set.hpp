...
#ifndef HAMON_FLAT_SET_FLAT_SET_HPP
...
// TODO:
// https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2023/p2767r2.html
// https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2025/p3567r0.html
...
public:
	...
	using iterator                  = hamon::detail::flat_set_iterator<flat_set, KeyContainer>;
	using const_iterator            = hamon::detail::flat_set_iterator<flat_set, KeyContainer>;
	...
	flat_set() HAMON_NOEXCEPT_IF(	// noexcept as an extension
	...
	flat_set(flat_set&& other) HAMON_NOEXCEPT_IF(	// noexcept as an extension
	...
	flat_set(key_compare const& comp)
		: c(), compare(comp)
	{}

	HAMON_CXX14_CONSTEXPR explicit
	flat_set(container_type cont, key_compare const& comp = key_compare())
	...
	HAMON_CXX14_CONSTEXPR
	flat_set(hamon::sorted_unique_t, container_type cont, key_compare const& comp = key_compare())
		: c(hamon::move(cont)), compare(comp)
	{
		// [flat.set.overview]/9
		HAMON_ASSERT(this->is_sorted_and_unique(c));
	}

	template <HAMON_CONSTRAINT(hamon::detail::cpp17_input_iterator, InputIterator)>
	HAMON_CXX14_CONSTEXPR
	flat_set(InputIterator first, InputIterator last, key_compare const& comp = key_compare())
		: c(), compare(comp)
	{
	...
	template <HAMON_CONSTRAINT(hamon::detail::cpp17_input_iterator, InputIterator)>
	HAMON_CXX14_CONSTEXPR
	flat_set(hamon::sorted_unique_t, InputIterator first, InputIterator last, key_compare const& comp = key_compare())
		: c(), compare(comp)
	{
	...
	HAMON_CXX14_CONSTEXPR
	flat_set(hamon::from_range_t, R&& rg)
		: flat_set(hamon::from_range, hamon::forward<R>(rg), key_compare())
	{}

	template <HAMON_CONSTRAINED_PARAM(hamon::detail::container_compatible_range, value_type, R)>
	HAMON_CXX14_CONSTEXPR
	flat_set(hamon::from_range_t, R&& rg, key_compare const& comp)
		: flat_set(comp)
	{
	...
	template <typename Alloc,
		typename = hamon::enable_if_t<
			hamon::uses_allocator<container_type, Alloc>::value>>		// [flat.set.cons.alloc]/1
	HAMON_CXX14_CONSTEXPR
	flat_set(container_type const& cont, Alloc const& a)
		: flat_set(cont, key_compare(), a)
	{}
	...
	template <typename Alloc,
		typename = hamon::enable_if_t<
			hamon::uses_allocator<container_type, Alloc>::value>>		// [flat.set.cons.alloc]/1
	HAMON_CXX14_CONSTEXPR
	flat_set(container_type const& cont, key_compare const& comp, Alloc const& a)
	...
	template <typename Alloc,
		typename = hamon::enable_if_t<
			hamon::uses_allocator<container_type, Alloc>::value>>		// [flat.set.cons.alloc]/1
	HAMON_CXX14_CONSTEXPR
	flat_set(hamon::sorted_unique_t, container_type const& cont, Alloc const& a)
		: flat_set(hamon::sorted_unique, cont, key_compare(), a)
	{}
	...
	template <typename Alloc,
		typename = hamon::enable_if_t<
			hamon::uses_allocator<container_type, Alloc>::value>>		// [flat.set.cons.alloc]/1
	HAMON_CXX14_CONSTEXPR
	flat_set(hamon::sorted_unique_t, container_type const& cont, key_compare const& comp, Alloc const& a)
		: c(hamon::make_obj_using_allocator<container_type>(a, cont))
		, compare(comp)
	{
		// [flat.set.overview]/9
	...
	template <typename Alloc,
		typename = hamon::enable_if_t<
			hamon::uses_allocator<container_type, Alloc>::value>>		// [flat.set.cons.alloc]/1
	HAMON_CXX14_CONSTEXPR
	flat_set(flat_set const& x, Alloc const& a)
	...
	template <typename Alloc,
		typename = hamon::enable_if_t<
			hamon::uses_allocator<container_type, Alloc>::value>>		// [flat.set.cons.alloc]/1
	HAMON_CXX14_CONSTEXPR
	flat_set(flat_set&& x, Alloc const& a)
	...
	flat_set(InputIterator first, InputIterator last, Alloc const& a)
		: flat_set(hamon::move(first), hamon::move(last), key_compare(), a)
	{}
	...
	template <HAMON_CONSTRAINT(hamon::detail::cpp17_input_iterator, InputIterator),
		...
		typename = hamon::enable_if_t<
			hamon::uses_allocator<container_type, Alloc>::value>>		// [flat.set.cons.alloc]/1
	HAMON_CXX14_CONSTEXPR
	flat_set(InputIterator first, InputIterator last, key_compare const& comp, Alloc const& a)
		: c(hamon::make_obj_using_allocator<container_type>(a))
		, compare(comp)
	{
		this->insert(hamon::move(first), hamon::move(last));
	...
	template <HAMON_CONSTRAINT(hamon::detail::cpp17_input_iterator, InputIterator),
		...
		typename = hamon::enable_if_t<
			hamon::uses_allocator<container_type, Alloc>::value>>		// [flat.set.cons.alloc]/1
	HAMON_CXX14_CONSTEXPR
	flat_set(hamon::sorted_unique_t, InputIterator first, InputIterator last, Alloc const& a)
		: flat_set(hamon::sorted_unique, hamon::move(first), hamon::move(last), key_compare(), a)
	{}
	...
	template <HAMON_CONSTRAINT(hamon::detail::cpp17_input_iterator, InputIterator),
		...
		typename = hamon::enable_if_t<
			hamon::uses_allocator<container_type, Alloc>::value>>		// [flat.set.cons.alloc]/1
	HAMON_CXX14_CONSTEXPR
	flat_set(hamon::sorted_unique_t, InputIterator first, InputIterator last, key_compare const& comp, Alloc const& a)
		: c(hamon::make_obj_using_allocator<container_type>(a))
		, compare(comp)
	{
		this->insert(hamon::sorted_unique, hamon::move(first), hamon::move(last));
	...
	template <HAMON_CONSTRAINED_PARAM(hamon::detail::container_compatible_range, value_type, R),
		...
		typename = hamon::enable_if_t<
			hamon::uses_allocator<container_type, Alloc>::value>>		// [flat.set.cons.alloc]/1
	HAMON_CXX14_CONSTEXPR
	flat_set(hamon::from_range_t, R&& rg, Alloc const& a)
		: flat_set(hamon::from_range, hamon::forward<R>(rg), key_compare(), a)
	{}
	...
	template <HAMON_CONSTRAINED_PARAM(hamon::detail::container_compatible_range, value_type, R),
		...
		typename = hamon::enable_if_t<
			hamon::uses_allocator<container_type, Alloc>::value>>		// [flat.set.cons.alloc]/1
	HAMON_CXX14_CONSTEXPR
	flat_set(hamon::from_range_t, R&& rg, key_compare const& comp, Alloc const& a)
		: c(hamon::make_obj_using_allocator<container_type>(a))
		, compare(comp)
	{
		this->insert_range(hamon::forward<R>(rg));
	...
	HAMON_CXX14_CONSTEXPR flat_set&
	operator=(flat_set&& other) HAMON_NOEXCEPT_IF(	// noexcept as an extension
	...
	template <typename K,
		...
		typename = hamon::enable_if_t<
			hamon::is_constructible<value_type, K>::value>>
	HAMON_CXX14_CONSTEXPR hamon::pair<iterator, bool>
	insert(K&& x)
	{
		return this->try_emplace_impl(hamon::forward<K>(x));
	}
	...
	template <typename K,
		...
		typename = hamon::enable_if_t<
			hamon::is_constructible<value_type, K>::value>>
	HAMON_CXX14_CONSTEXPR iterator
	insert(const_iterator hint, K&& x)
	{
		return this->try_emplace_hint_impl(hamon::move(hint), hamon::forward<K>(x)).first;
	}
	...
	{
		HAMON_ASSERT(this->is_sorted_and_unique(cont));
...
//		requires (!hamon::same_as<iterator, const_iterator>)
	...
	HAMON_CXX14_CONSTEXPR void
	swap(flat_set& y) HAMON_NOEXCEPT
	{
	...
	operator==(flat_set const& x, flat_set const& y)
	{
...
//	hamon::detail::synth_three_way_result<value_type>
	...
	swap(flat_set& x, flat_set& y) HAMON_NOEXCEPT
	{
		x.swap(y);
	}
...
private:
	...
	unique()
	{
		auto it = hamon::ranges::unique(c, key_equiv(this->compare)).begin();
		c.erase(it, c.end());
	}
	...
	append_merge_unique(Iterator first, Sentinel last)
	{
		...
		HAMON_ASSERT(this->is_sorted_and_unique(c | hamon::views::drop(m)));
	...
	{
		return this->try_emplace_hint_impl(hamon::move(position), hamon::forward<Arg>(arg)).first;
	}
	...
	{
		return this->try_emplace_hint_impl(hamon::move(position), Key(hamon::forward<Args>(args)...)).first;
	}
	...
	{
		auto key_iter = this->lower_bound(k);
		return this->try_emplace_pos(hamon::move(key_iter), hamon::forward<K>(k));
	}
	...
	{
		auto prev_larger  = hint != cbegin() && !this->compare(*hamon::ranges::prev(hint), k);
		auto next_smaller = hint != cend() && this->compare(*hint, k);

		if (!prev_larger && !next_smaller)
		{
			// hint correct
			return this->try_emplace_pos(hamon::move(hint), hamon::forward<K>(k));
		}

		return this->try_emplace_impl(hamon::forward<K>(k));
	}

	template <typename K>
	HAMON_CXX14_CONSTEXPR hamon::pair<iterator, bool>
	try_emplace_pos(iterator key_iter, K&& k)
	{
		if (key_iter == end() || this->compare(k, *key_iter))
		{
			return this->emplace_exact_pos(hamon::move(key_iter), hamon::forward<K>(k));
		}

		return hamon::pair<iterator, bool>(hamon::move(key_iter), false);
	}
	...
	emplace_exact_pos(iterator key_iter, K&& k)
	{
		...
		return hamon::pair<iterator, bool>(hamon::move(key_iter), true);
	}
	...
	{
		auto it_first = self.lower_bound(k);
		auto it_last = it_first;
		if (!(it_first == self.end() || self.compare(k, *it_first)))
		{
			it_last = hamon::ranges::next(it_first);
		}
		return hamon::pair<It, It> { it_first, it_last };
	}
...
template <
	...
	>::value>
>
flat_set(hamon::sorted_unique_t, KeyContainer, Compare = Compare())
->flat_set<
	typename KeyContainer::value_type,
	Compare,
	KeyContainer>;
...
template <
	...
	>::value>
>
flat_set(hamon::sorted_unique_t, KeyContainer, Allocator)
->flat_set<
	typename KeyContainer::value_type,
	hamon::less<typename KeyContainer::value_type>,
	KeyContainer>;
...
template <
	...
	>::value>
>
flat_set(hamon::sorted_unique_t, KeyContainer, Compare, Allocator)
->flat_set<
	typename KeyContainer::value_type,
	Compare,
	KeyContainer>;
...
template <
	...
	>
>
flat_set(hamon::sorted_unique_t, InputIterator, InputIterator, Compare = Compare())
->flat_set<
	hamon::detail::iter_value_type<InputIterator>,
	Compare>;
...
template <
	...
	>::value>
>
flat_set(hamon::from_range_t, R&&, Compare = Compare(), Allocator = Allocator())
->flat_set<
...
template <
	...
	>::value>
>
flat_set(hamon::from_range_t, R&&, Allocator)
->flat_set<
	ranges::range_value_t<R>,
...
HAMON_CXX14_CONSTEXPR typename flat_set<Key, Compare, KeyContainer>::size_type
...
struct uses_allocator<hamon::flat_set<Key, Compare, KeyContainer>, Allocator>
