/**
 *	@file	unit_test_flat_set_insert_first_last.cpp
 *
 *	@brief	insert のテスト
 *
...
FLAT_SET_TEST_CONSTEXPR bool test_impl()
{
	...
	static_assert(hamon::is_same<decltype(hamon::declval<Set&>().insert(hamon::sorted_unique, hamon::declval<InputIterator>(), hamon::declval<InputIterator>())), void>::value, "");
	static_assert(!noexcept(hamon::declval<Set&>().insert(hamon::declval<InputIterator>(), hamon::declval<InputIterator>())), "");
	static_assert(!noexcept(hamon::declval<Set&>().insert(hamon::sorted_unique, hamon::declval<InputIterator>(), hamon::declval<InputIterator>())), "");
	...
	}
	{
		Set v;
		{
			...
			v.insert(hamon::sorted_unique, InputIterator{hamon::ranges::begin(a)}, InputIterator{hamon::ranges::end(a)});
		}
		VERIFY(check_invariant(v));
		VERIFY(v.size() == 3);
		VERIFY(v.count(Key{0}) == 0);
