/**
 *	@file	unit_test_flat_set_pmr.cpp
 *
 *	@brief	flat_set のテスト
 */
...
GTEST_TEST(FlatSetTest, PmrTest)
{
	{
		// flat_set(const Allocator& a);
		using M = hamon::flat_set<int, hamon::less<int>, hamon::pmr::vector<int>>;
	...
	}
	{
		...
		using M = hamon::flat_set<int, std::function<bool(int, int)>, hamon::pmr::vector<int>>;
		hamon::pmr::monotonic_buffer_resource mr;
	...
	{
		// flat_set(const key_container_type& key_cont, const Allocator& a);
		using M = hamon::flat_set<int, hamon::less<int>, hamon::pmr::vector<int>>;
		hamon::pmr::monotonic_buffer_resource mr;
		hamon::pmr::vector<M> vm(&mr);
		...
		EXPECT_TRUE((vm[0] == M {1, 2, 3}));
		EXPECT_TRUE(hamon::move(vm[0]).extract().get_allocator().resource() == &mr);
	}
	{
		// flat_set(const flat_set&, const allocator_type&);
		using C = TestLess<int>;
		using M = hamon::flat_set<int, C, hamon::pmr::vector<int>>;
	...
	{
		// flat_set(const flat_set&, const allocator_type&);
		using M = hamon::flat_set<int, hamon::less<>, hamon::pmr::vector<int>>;
	...
	}
	{
		...
		using M = hamon::flat_set<int, hamon::less<>, hamon::pmr::deque<int>>;
		hamon::pmr::monotonic_buffer_resource mr1;
		hamon::pmr::monotonic_buffer_resource mr2;
		M mo = M({1, 2, 3}, &mr1);
		M m = M({4, 5}, &mr2);
	...
	}
	{
		...
		EXPECT_TRUE((hamon::move(vs[0]).extract() == hamon::pmr::deque<int>{1, 2, 3}));
	}
	{
		// flat_set& operator=(flat_set&&);
		using M = hamon::flat_set<hamon::pmr::string, hamon::less<>, hamon::pmr::vector<hamon::pmr::string>>;
		hamon::pmr::monotonic_buffer_resource mr1;
		hamon::pmr::monotonic_buffer_resource mr2;
	...
	}
	{
		...
		vm.emplace_back(hamon::sorted_unique, ks);
		EXPECT_TRUE(!ks.empty()); // it was an lvalue above
		EXPECT_TRUE((vm[0] == M {1, 2, 4, 10}));
		EXPECT_TRUE(hamon::move(vm[0]).extract().get_allocator().resource() == &mr);
	}
	{
		// flat_set(sorted_unique_t, const container_type& key_cont,const Alloc& a);
	...
	}
	{
		...
		int ar[] = {1, 2, 4, 5};
		using Iter = cpp17_input_iterator_wrapper<int>;
		vm.emplace_back(hamon::sorted_unique, Iter{ar}, Iter{ar + 4}, C(3));
		EXPECT_TRUE((vm[0] == M {1, 2, 4, 5}));
	...
	}
	{
		...
		hamon::pmr::vector<M> vm(&mr);
		int ar[1] = {42};
		vm.emplace_back(hamon::sorted_unique, &ar[0], &ar[0], C(4));
		EXPECT_TRUE(vm[0] == M {});
