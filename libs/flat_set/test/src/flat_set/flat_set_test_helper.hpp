/**
 *	@file	flat_set_test_helper.hpp
 *
 *	@brief
 */

#ifndef HAMON_FLAT_SET_TEST_HELPER_HPP
...
check_invariant(FlatSet const& v)
{
	if (!hamon::is_sorted(v.begin(), v.end(), v.key_comp()))
	{
		return false;
	}

	using KeyCompare = typename FlatSet::key_compare;
	using KeyType = typename FlatSet::key_type;

	struct KeyEqual
	{
		HAMON_CXX11_CONSTEXPR
		KeyEqual(KeyCompare c)
			: comp(c)
		{}

		HAMON_CXX11_CONSTEXPR bool
		operator()(KeyType const& x, KeyType const& y) const
		{
	...
	if (!hamon::is_unique(v.begin(), v.end(), KeyEqual(v.key_comp())))
	{
		return false;
	}
...
#endif // HAMON_FLAT_SET_TEST_HELPER_HPP
