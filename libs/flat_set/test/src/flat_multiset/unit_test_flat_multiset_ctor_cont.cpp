/**
 *	@file	unit_test_flat_multiset_ctor_cont.cpp
 *
...
FLAT_SET_TEST_CONSTEXPR bool test()
{
	...
	static_assert( hamon::is_constructible<Set, hamon::sorted_equivalent_t, KeyContainer>::value, "");
	static_assert( hamon::is_constructible<Set, hamon::sorted_equivalent_t, KeyContainer, Compare const&>::value, "");
	...
	static_assert(!hamon::is_nothrow_constructible<Set, hamon::sorted_equivalent_t, KeyContainer>::value, "");
	static_assert(!hamon::is_nothrow_constructible<Set, hamon::sorted_equivalent_t, KeyContainer, Compare const&>::value, "");
	...
	static_assert( hamon::is_implicitly_constructible<Set, hamon::sorted_equivalent_t, KeyContainer>::value, "");
	static_assert( hamon::is_implicitly_constructible<Set, hamon::sorted_equivalent_t, KeyContainer, Compare const&>::value, "");
	...
	static_assert(!hamon::is_trivially_constructible<Set, hamon::sorted_equivalent_t, KeyContainer>::value, "");
	static_assert(!hamon::is_trivially_constructible<Set, hamon::sorted_equivalent_t, KeyContainer, Compare const&>::value, "");
	...
	}
	{
		...
		hamon::flat_multiset<Key, TestLess<Key>, KeyContainer> v{hamon::move(cont), comp};
		VERIFY(check_invariant(v));
		VERIFY(v.size() == 6);
	...
	{
		KeyContainer cont {Key{3}, Key{1}, Key{4}, Key{4}};
		hamon::ranges::sort(cont, Compare{});
		Set v{hamon::sorted_equivalent, hamon::move(cont)};
		VERIFY(check_invariant(v));
		VERIFY(!v.empty());
		VERIFY(v.size() == 4);
		VERIFY(v.count(Key{0}) == 0);
	...
	{
		KeyContainer cont {Key{3}, Key{1}, Key{3}, Key{3}, Key{4}, Key{2}, Key{1}};
		TestLess<Key> const comp{42};
		hamon::ranges::sort(cont, comp);
		hamon::flat_multiset<Key, TestLess<Key>, KeyContainer> v{hamon::sorted_equivalent, cont, comp};
		VERIFY(check_invariant(v));
		VERIFY(v.size() == 7);
		VERIFY(v.key_comp() == comp);
		auto it = v.begin();
...
GTEST_TEST(FlatMultisetTest, CtorContTest)
{
