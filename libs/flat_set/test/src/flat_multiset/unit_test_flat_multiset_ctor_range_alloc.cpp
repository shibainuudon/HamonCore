/**
 *	@file	unit_test_flat_multiset_ctor_range_alloc.cpp
 *
...
FLAT_SET_TEST_CONSTEXPR bool test_impl()
{
	...
	}
	{
		...
		Alloc const alloc;
		hamon::flat_multiset<Key, TestLess<Key>, KeyContainer> v{hamon::from_range, r, comp, alloc};
		VERIFY(check_invariant(v));
		VERIFY(!v.empty());
		VERIFY(v.size() == 5);
		auto it = v.begin();
...
GTEST_TEST(FlatMultisetTest, CtorRangeAllocTest)
{
