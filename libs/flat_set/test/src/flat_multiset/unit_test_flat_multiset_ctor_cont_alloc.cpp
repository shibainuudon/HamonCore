/**
 *	@file	unit_test_flat_multiset_ctor_cont_alloc.cpp
 *
...
FLAT_SET_TEST_CONSTEXPR bool test()
{
	...
	static_assert( hamon::is_constructible<Set, hamon::sorted_equivalent_t, KeyContainer, Alloc const&>::value, "");
	static_assert( hamon::is_constructible<Set, hamon::sorted_equivalent_t, KeyContainer, Compare const&, Alloc const&>::value, "");
	...
	static_assert(!hamon::is_nothrow_constructible<Set, hamon::sorted_equivalent_t, KeyContainer, Alloc const&>::value, "");
	static_assert(!hamon::is_nothrow_constructible<Set, hamon::sorted_equivalent_t, KeyContainer, Compare const&, Alloc const&>::value, "");
	...
	static_assert( hamon::is_implicitly_constructible<Set, hamon::sorted_equivalent_t, KeyContainer, Alloc const&>::value, "");
	static_assert( hamon::is_implicitly_constructible<Set, hamon::sorted_equivalent_t, KeyContainer, Compare const&, Alloc const&>::value, "");
	...
	static_assert(!hamon::is_trivially_constructible<Set, hamon::sorted_equivalent_t, KeyContainer, Alloc const&>::value, "");
	static_assert(!hamon::is_trivially_constructible<Set, hamon::sorted_equivalent_t, KeyContainer, Compare const&, Alloc const&>::value, "");
	...
	}
	{
		...
		Alloc const alloc;
		hamon::flat_multiset<Key, TestLess<Key>, KeyContainer> v{key_cont, comp, alloc};
		VERIFY(check_invariant(v));
		VERIFY(v.size() == 6);
	...
	{
		KeyContainer key_cont {Key{3}, Key{1}, Key{4}, Key{1}, Key{4}};
		hamon::ranges::sort(key_cont, Compare{});
		Alloc const alloc;
		Set v{hamon::sorted_equivalent, key_cont, alloc};
		VERIFY(check_invariant(v));
		VERIFY(!v.empty());
		VERIFY(v.size() == 5);
		VERIFY(v.count(Key{0}) == 0);
	...
	{
		KeyContainer key_cont {Key{3}, Key{3}, Key{1}, Key{4}, Key{2}};
		TestLess<Key> const comp{42};
		hamon::ranges::sort(key_cont, comp);
		Alloc const alloc;
		hamon::flat_multiset<Key, TestLess<Key>, KeyContainer> v{hamon::sorted_equivalent, key_cont, comp, alloc};
		VERIFY(check_invariant(v));
		VERIFY(v.size() == 5);
		VERIFY(v.key_comp() == comp);
		auto it = v.begin();
...
GTEST_TEST(FlatMultisetTest, CtorContAllocTest)
{
