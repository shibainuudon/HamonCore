/**
 *	@file	unit_test_chrono_time_point_increment.cpp
 *
 *	@brief	operator++() のテスト
 *
...
namespace time_point_increment_test
{
...
HAMON_CXX14_CONSTEXPR bool test()
{
	...
	{
		auto tp2 = ++tp;
		VERIFY(tp.time_since_epoch().count() == 11);
		VERIFY(tp2.time_since_epoch().count() == 11);
	}
	{
		auto tp2 = tp++;
...
GTEST_TEST(TimePointTest, IncrementTest)
{
