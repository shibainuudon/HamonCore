...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_abs =
	hamon::detail::class_or_enum<T> &&
	requires(T const& t)
	{
		{ abs(t) } -> hamon::detail::is_not_void;
	};
...
private:
	template <typename U,
		...
		typename S = decltype(abs(hamon::declval<U const&>())),
...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_abs = has_adl_abs_impl<T>::type::value;
