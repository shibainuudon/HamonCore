...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_sin =
	hamon::detail::class_or_enum<T> &&
	requires(T const& t)
	{
		{ sin(t) } -> hamon::detail::is_not_void;
	};
...
private:
	template <typename U,
		...
		typename S = decltype(sin(hamon::declval<U const&>())),
...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_sin = has_adl_sin_impl<T>::type::value;
