...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_fabs =
	hamon::detail::class_or_enum<T> &&
	requires(T const& t)
	{
		{ fabs(t) } -> hamon::detail::is_not_void;
	};
...
private:
	template <typename U,
		...
		typename S = decltype(fabs(hamon::declval<U const&>())),
...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_fabs = has_adl_fabs_impl<T>::type::value;
