...
HAMON_CONCEPT_OR_BOOL has_adl_smoothstep =
	...
	{
		{ smoothstep(x, y, z) } -> hamon::detail::is_not_void;
	};
...
template <typename T1, typename T2, typename T3>
HAMON_CONCEPT_OR_BOOL has_adl_smoothstep = has_adl_smoothstep_impl<T1, T2, T3>::type::value;
