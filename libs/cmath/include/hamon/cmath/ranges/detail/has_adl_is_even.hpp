...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_is_even =
	hamon::detail::class_or_enum<T> &&
	requires(T const& t)
	{
		{ is_even(t) } -> hamon::detail::is_not_void;
	};
...
private:
	template <typename U,
		...
		typename S = decltype(is_even(hamon::declval<U const&>())),
...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_is_even = has_adl_is_even_impl<T>::type::value;
