...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_isinf =
	hamon::detail::class_or_enum<T> &&
	requires(T const& t)
	{
		{ isinf(t) } -> hamon::detail::is_not_void;
	};
...
private:
	template <typename U,
		...
		typename S = decltype(isinf(hamon::declval<U const&>())),
...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_isinf = has_adl_isinf_impl<T>::type::value;
