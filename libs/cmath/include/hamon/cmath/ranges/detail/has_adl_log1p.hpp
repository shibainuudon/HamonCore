...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_log1p =
	hamon::detail::class_or_enum<T> &&
	requires(T const& t)
	{
		{ log1p(t) } -> hamon::detail::is_not_void;
	};
...
private:
	template <typename U,
		...
		typename S = decltype(log1p(hamon::declval<U const&>())),
...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_log1p = has_adl_log1p_impl<T>::type::value;
