...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_floor =
	hamon::detail::class_or_enum<T> &&
	requires(T const& t)
	{
		{ floor(t) } -> hamon::detail::is_not_void;
	};
...
private:
	template <typename U,
		...
		typename S = decltype(floor(hamon::declval<U const&>())),
...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_floor = has_adl_floor_impl<T>::type::value;
