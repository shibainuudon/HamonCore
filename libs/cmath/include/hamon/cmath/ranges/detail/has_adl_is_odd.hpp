...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_is_odd =
	hamon::detail::class_or_enum<T> &&
	requires(T const& t)
	{
		{ is_odd(t) } -> hamon::detail::is_not_void;
	};
...
private:
	template <typename U,
		...
		typename S = decltype(is_odd(hamon::declval<U const&>())),
...
template <typename T>
HAMON_CONCEPT_OR_BOOL has_adl_is_odd = has_adl_is_odd_impl<T>::type::value;
