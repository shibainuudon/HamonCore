...
bessel_ik(T nu, T x, T& Inu, T& Knu, T& Ipnu, T& Kpnu)
{
...

	{
		...
		//if (i > max_iter)
		//{
		//	hamon::detail::throw_runtime_error("Argument x too large in bessel_ik; try asymptotic expansion.");
		//}
	}
	...
	if (x < x_min)
	{
		...
		//if (i > max_iter)
		//{
		//	hamon::detail::throw_runtime_error("Bessel k series failed to converge in bessel_ik.");
		//}
		Kmu = sum;
		Knu1 = sum1 * xi2;
	}
	else
	{
		...
		//if (i > max_iter)
		//{
		//	hamon::detail::throw_runtime_error("Steed's method failed in bessel_ik.");
		//}
		h2 = a1 * h2;
		Kmu = hamon::sqrt(hamon::numbers::pi_fn<T>() / (T(2) * x)) * hamon::exp(-x) / s;
