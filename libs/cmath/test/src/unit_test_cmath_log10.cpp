...
static_assert(std::is_same<float,       decltype(hamon::log10(0.0f))>::value, "");
static_assert(std::is_same<float,       decltype(hamon::log10f(0.0f))>::value, "");
static_assert(std::is_same<double,      decltype(hamon::log10(0.0 ))>::value, "");
static_assert(std::is_same<double,      decltype(hamon::log10(0   ))>::value, "");
static_assert(std::is_same<long double, decltype(hamon::log10(0.0l))>::value, "");
static_assert(std::is_same<long double, decltype(hamon::log10l(0.0l))>::value, "");

template <typename T>
double log10_error();

template <>
inline HAMON_CXX11_CONSTEXPR double log10_error<float>()
{
	return 0.000001;
}

template <>
inline HAMON_CXX11_CONSTEXPR double log10_error<double>()
{
	return 0.000000000001;
}

template <>
inline HAMON_CXX11_CONSTEXPR double log10_error<long double>()
{
	return 0.000000000001;
}
...
void Log10TestFloat()
{
	...
	HAMON_CXX11_CONSTEXPR double error = log10_error<T>();
