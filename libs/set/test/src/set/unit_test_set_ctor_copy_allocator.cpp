/**
 *	@file	unit_test_set_ctor_copy_allocator.cpp
 *
...
SET_TEST_CONSTEXPR bool test_impl(Compare const& comp, Allocator const& alloc)
{
	...
	static_assert( hamon::is_constructible<Set, Set const&, Allocator const&>::value, "");
	static_assert(!hamon::is_nothrow_constructible<Set, Set const&, Allocator const&>::value, "");
	static_assert( hamon::is_implicitly_constructible<Set, Set const&, Allocator const&>::value, "");
	static_assert(!hamon::is_trivially_constructible<Set, Set const&, Allocator const&>::value, "");
...
GTEST_TEST(SetTest, CtorCopyAllocatorTest)
{
	...
	S::s_dtor_count = 0;
	{
		...
		hamon::set<S, hamon::less<>, MyAllocator1<S>> v2{v1, MyAllocator1<S>{10}};
		EXPECT_EQ(4, S::s_ctor_count);
	...
	S::s_dtor_count = 0;
	{
		...
		hamon::set<S, hamon::less<>, MyAllocator1<S>> v2{v1, MyAllocator1<S>{20}};
		EXPECT_EQ(4, S::s_ctor_count);
		EXPECT_EQ(3, S::s_copy_ctor_count);
		EXPECT_EQ(0, S::s_move_ctor_count);
		EXPECT_EQ(1, S::s_dtor_count);
