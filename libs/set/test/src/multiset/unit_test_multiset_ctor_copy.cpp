/**
 *	@file	unit_test_multiset_ctor_copy.cpp
 *
...
MULTISET_TEST_CONSTEXPR bool test_impl(Compare const& comp, Allocator const& alloc, bool propagate_allocator)
{
...
MULTISET_TEST_CONSTEXPR bool test1()
{
	...
	VERIFY(test_impl<Key>(comp, alloc, true));
...
MULTISET_TEST_CONSTEXPR bool test3()
{
	...
	VERIFY(test_impl<Key>(comp, alloc, false));
...
GTEST_TEST(MultisetTest, CtorCopyTest)
{
	...
	S::s_dtor_count = 0;
	{
		...
		hamon::multiset<S, hamon::less<>, MyAllocator1<S>> v2{v1};
		EXPECT_EQ(4, S::s_ctor_count);
