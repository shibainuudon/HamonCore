/**
 *	@file	unit_test_multiset_upper_bound_heterogeneous.cpp
 *
...
MULTISET_TEST_CONSTEXPR bool test2()
{
	...
	VERIFY(v.upper_bound(0) == hamon::next(v.begin(), 1));
	VERIFY(v.upper_bound(1) == hamon::next(v.begin(), 3));
	VERIFY(v.upper_bound(2) == hamon::next(v.begin(), 3));
	VERIFY(v.upper_bound(3) == hamon::next(v.begin(), 3));
	VERIFY(v.upper_bound(4) == hamon::next(v.begin(), 4));
...
GTEST_TEST(MultisetTest, UpperBoundHeterogeneousTest)
{
