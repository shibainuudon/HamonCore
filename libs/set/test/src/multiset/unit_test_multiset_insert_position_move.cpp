/**
 *	@file	unit_test_multiset_insert_position_move.cpp
 *
 *	@brief	insert のテスト
 *
...
MULTISET_TEST_CONSTEXPR bool test1()
{
	...
	{
		auto r = v.insert(v.end(), ValueType{15});
	...
	{
		auto r = v.insert(v.end(), ValueType{20});
...
MULTISET_TEST_CONSTEXPR bool test2()
{
	...
	{
		auto r = v.insert(v.begin(), S1{1, 20});
		VERIFY(r == hamon::next(v.begin(), 0));
	...
	{
		auto r = v.insert(hamon::next(v.begin(), 1), S1{2, 10});
		VERIFY(r == hamon::next(v.begin(), 2));
	...
	{
		auto r = v.insert(v.begin(), S1{3, 20});
...
GTEST_TEST(MultisetTest, InsertPositionMoveTest)
{
