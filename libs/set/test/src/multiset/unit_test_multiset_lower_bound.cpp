/**
 *	@file	unit_test_multiset_lower_bound.cpp
 *
...
MULTISET_TEST_CONSTEXPR bool test1()
{
	...
	static_assert(hamon::is_same<decltype(hamon::declval<Set&>().lower_bound(hamon::declval<Key const&>())), Iterator>::value, "");
...
MULTISET_TEST_CONSTEXPR bool test2()
{
	...
	static_assert(hamon::is_same<decltype(hamon::declval<Set const&>().lower_bound(hamon::declval<Key const&>())), ConstIterator>::value, "");
	static_assert(!noexcept(hamon::declval<Set const&>().lower_bound(hamon::declval<Key const&>())), "");

	Set const v {Key{1}, Key{3}, Key{3}, Key{1}, Key{4}, Key{1}};

	VERIFY(v.lower_bound(Key{0}) == hamon::next(v.begin(), 0));
	VERIFY(v.lower_bound(Key{1}) == hamon::next(v.begin(), 0));
	VERIFY(v.lower_bound(Key{2}) == hamon::next(v.begin(), 3));
	VERIFY(v.lower_bound(Key{3}) == hamon::next(v.begin(), 3));
	VERIFY(v.lower_bound(Key{4}) == hamon::next(v.begin(), 5));
	VERIFY(v.lower_bound(Key{5}) == v.end());
...
MULTISET_TEST_CONSTEXPR bool test4()
{
	...
	VERIFY(v.lower_bound(1) == hamon::next(v.begin(), 1));
	VERIFY(v.lower_bound(2) == hamon::next(v.begin(), 3));
	VERIFY(v.lower_bound(3) == hamon::next(v.begin(), 3));
	VERIFY(v.lower_bound(4) == hamon::next(v.begin(), 3));
	VERIFY(v.lower_bound(5) == hamon::next(v.begin(), 4));
...
GTEST_TEST(MultisetTest, LowerBoundTest)
{
