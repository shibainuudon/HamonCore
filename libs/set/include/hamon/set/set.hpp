...
private:
	using Tree = hamon::detail::red_black_tree<false, value_type, key_type, size_type, difference_type>;
...
public:
	...
	insert(node_type&& nh)
	{
		auto r = m_impl.insert_node(m_comp, nh.ptr());
		if (r.second)
		{
			nh.release();
		}
	...
	{
		auto r = m_impl.insert_node_hint(m_comp, hint, nh.ptr());
		if (r.second)
		{
			nh.release();
		}
