/**
 *	@file	unit_test_tuple_tuple_ctor_alloc_tuple_crref.cpp
 *
...
namespace ctor_alloc_tuple_crref_test
{
...
GTEST_TEST(TupleTest, CtorAllocTupleCRRefTest)
{
	HAMON_CXX11_CONSTEXPR MyAlloc a{};
	{
		hamon::tuple<char> const t1(42);
		hamon::tuple<int>  const t2(hamon::allocator_arg, a, hamon::move(t1));
		EXPECT_TRUE(hamon::adl_get<0>(t2) == 42);
	}
	{
		hamon::tuple<const char*, int>  const t1("hello", 43);
		hamon::tuple<hamon::string, long> const t2(hamon::allocator_arg, a, hamon::move(t1));
		EXPECT_TRUE(hamon::adl_get<0>(t2) == "hello");
		EXPECT_TRUE(hamon::adl_get<1>(t2) == 43);
	}
	{
		hamon::tuple<int>                const t1(10);
		hamon::tuple<ImplicitFirstAlloc> const t2(hamon::allocator_arg, a, hamon::move(t1));
		EXPECT_TRUE(hamon::adl_get<0>(t2).n == 10);
	}
	{
		hamon::tuple<int, int>               const t1(11, 12);
		hamon::tuple<ImplicitLastAlloc, int> const t2(hamon::allocator_arg, a, hamon::move(t1));
		EXPECT_TRUE(hamon::adl_get<0>(t2).n == 11);
		EXPECT_TRUE(hamon::adl_get<1>(t2) == 12);
	}
	{
		hamon::tuple<int, int, int>                              const t1(3, 4, 5);
		hamon::tuple<ImplicitNoAlloc, float, ExplicitFirstAlloc> const t2(hamon::allocator_arg, a, hamon::move(t1));
	...
	}
	{
		...
		HAMON_CXX11_CONSTEXPR hamon::tuple<int>  const t2(hamon::allocator_arg, a, hamon::move(t1));
		HAMON_CXX11_CONSTEXPR_EXPECT_TRUE(hamon::adl_get<0>(t2) == 42);
	}
	{
		HAMON_CXX11_CONSTEXPR hamon::tuple<float, int> const t1(43.0f, 44);
		HAMON_CXX11_CONSTEXPR hamon::tuple<double, long> const t2(hamon::allocator_arg, a, hamon::move(t1));
