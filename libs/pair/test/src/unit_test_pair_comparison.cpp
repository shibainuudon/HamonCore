...
GTEST_TEST(PairTest, ComparisonTest)
{
	...
	}
	{
		int i = 1;
		int j = 2;
		hamon::pair<int,        int>        p1{1, 2};
		hamon::pair<int,        int const&> p2{1, j};
		hamon::pair<int,        int>        p3{0, 2};
		hamon::pair<int const&, int>        p4{j, 1};
		hamon::pair<int const&, int const&> p5{i, i};
		hamon::pair<int,        int>        p6{1, 3};
...
#if defined(HAMON_HAS_CXX20_THREE_WAY_COMPARISON)
		EXPECT_TRUE(hamon::is_eq(p1 <=> p1));
		EXPECT_TRUE(hamon::is_eq(p1 <=> p2));
		EXPECT_TRUE(hamon::is_gt(p1 <=> p3));
		EXPECT_TRUE(hamon::is_lt(p1 <=> p4));
		EXPECT_TRUE(hamon::is_gt(p1 <=> p5));
		EXPECT_TRUE(hamon::is_lt(p1 <=> p6));
#endif
	}
}
