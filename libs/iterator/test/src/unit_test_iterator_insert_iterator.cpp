/**
 *	@file	unit_test_iterator_insert_iterator.cpp
 *
 *	@brief	insert_iterator のテスト
 */
...
struct MyContainer
{
	...
	HAMON_CXX14_CONSTEXPR value_type const* insert(value_type const* it, value_type const& v)
	{
		m_arr[m_size] = v;
		++m_size;
		return it;
...
{
	using I = hamon::insert_iterator<Container>;
	...
	using CI = hamon::ranges::iterator_t<Container>;
	static_assert( hamon::is_constructible<I, Container&, CI>::value, "");
	static_assert(!hamon::is_constructible<I, Container&&, CI>::value, "");
	static_assert(!hamon::is_constructible<I, Container const&, CI>::value, "");
	static_assert(!hamon::is_constructible<I, Container const&&, CI>::value, "");
