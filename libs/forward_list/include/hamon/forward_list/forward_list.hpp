...
private:
	HAMON_CXX11_CONSTEXPR forward_list_node_base*
...
public:
	...
	using iterator        = hamon::detail::forward_list_iterator<T, Allocator, false>; // see [container.requirements]
	using const_iterator  = hamon::detail::forward_list_iterator<T, Allocator, true>;  // see [container.requirements]

	// [forward.list.cons], construct/copy/destroy
	HAMON_CXX11_CONSTEXPR
	forward_list() HAMON_NOEXCEPT_IF(hamon::is_nothrow_default_constructible<NodeAllocator>::value)	// noexcept as an extension
...
#define HAMON_IF_CONSTEXPR	if
	...
	forward_list(forward_list&& x, hamon::type_identity_t<Allocator> const& a)
		HAMON_NOEXCEPT_IF(hamon::allocator_traits<Allocator>::is_always_equal::value)	// noexcept as an extension
		: m_allocator(a)
	{
	...
	HAMON_CXX14_CONSTEXPR
	iterator erase_after(const_iterator position) HAMON_NOEXCEPT	// noexcept as an extension
	{
	...
	HAMON_CXX14_CONSTEXPR
	iterator erase_after(const_iterator position, const_iterator last) HAMON_NOEXCEPT	// noexcept as an extension
	{
	...
	HAMON_CXX14_CONSTEXPR
	void splice_after(const_iterator position, forward_list& x) HAMON_NOEXCEPT	// noexcept as an extension
	{
		// [forward.list.ops]/3
