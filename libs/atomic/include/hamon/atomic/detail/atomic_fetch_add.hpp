...
// Note:
// gcc の __atomic_fetch_add は浮動小数点数に対応していない(gcc-16 現在)
// SFINAEでテストしたかったが、引数の型違いの場合はハードエラーになってしまうようだ。
// (関数自体が無い場合はOK) 

// floating_point
#if HAMON_HAS_BUILTIN(__atomic_fetch_add) && !defined(HAMON_GCC)
template <HAMON_CONSTRAINT(hamon::floating_point, T)>
HAMON_CXX14_CONSTEXPR T
atomic_fetch_add_impl(T* ptr, T val, hamon::memory_order order, hamon::detail::overload_priority<3>)
{
...
#elif HAMON_HAS_BUILTIN(__atomic_fetch_add)
	return __atomic_fetch_add(ptr, val, hamon::detail::to_gcc_memory_order(order));
#else
	return atomic_fetch_add_impl(ptr, val, order, hamon::detail::overload_priority<0>{});
#endif
}
...
HAMON_CXX14_CONSTEXPR T*
atomic_fetch_add_impl(T** ptr, U val, hamon::memory_order order, hamon::detail::overload_priority<1>)
{
#if defined(HAMON_MSVC)
	(void)order;
...
#elif HAMON_HAS_BUILTIN(__atomic_fetch_add)
	return __atomic_fetch_add(ptr, val * static_cast<U>(sizeof(T)), hamon::detail::to_gcc_memory_order(order));
#else
	return atomic_fetch_add_impl(ptr, val, order, hamon::detail::overload_priority<0>{});
#endif
}
...
template <typename T, typename U>
HAMON_CXX14_CONSTEXPR T
atomic_fetch_add_impl(T* ptr, U val, hamon::memory_order order, hamon::detail::overload_priority<0>)
{
...
template <typename T, typename U>
HAMON_CXX14_CONSTEXPR T atomic_fetch_add(T* ptr, U val, hamon::memory_order order)
{
	...
	return hamon::detail::atomic_fetch_add_impl(ptr, val, order, hamon::detail::overload_priority<3>{});
}

template <typename T, typename U>
HAMON_CXX14_CONSTEXPR T atomic_fetch_add(T* ptr, U val)
{
