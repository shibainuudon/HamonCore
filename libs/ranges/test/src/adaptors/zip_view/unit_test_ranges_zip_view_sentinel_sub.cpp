/**
 *	@file	unit_test_ranges_zip_view_sentinel_sub.cpp
 *
 *	@brief	operator- のテスト
 *
 *	template<bool OtherConst>
 *	requires (sized_sentinel_for<
 *	  sentinel_t<maybe-const<Const, Views>>,
...
HAMON_CXX14_CONSTEXPR bool test00()
{
	...
	// iterator<false>, sentinel<true>
	{
		using V1 = TestView<int,
			TestIterator<int>,
			test_sentinel<TestIterator<int>>,
			TestIterator<int const>,
			test_sentinel<TestIterator<int const>>
		>;
...
GTEST_TEST(RangesTest, ZipViewSentinelSubTest)
{
