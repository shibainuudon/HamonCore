/**
 *	@file	unit_test_ranges_cache_latest_view_reserve_hint.cpp
 *
...
HAMON_CXX14_CONSTEXPR bool test00()
{
	//  approximately_sized_range<V> && approximately_sized_range<const V>
	{
		...
		static_assert( has_reserve_hint<CLV&>::value, "");
		static_assert( has_reserve_hint<CLV const&>::value, "");

		using D  = decltype(hamon::declval<CLV&>().reserve_hint());
		static_assert(hamon::is_same<D, int>::value, "");

		using CD = decltype(hamon::declval<CLV const&>().reserve_hint());
...
GTEST_TEST(RangesTest, CacheLatestViewReserveHintTest)
{
