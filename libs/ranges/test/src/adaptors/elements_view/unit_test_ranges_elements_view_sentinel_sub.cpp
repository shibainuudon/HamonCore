/**
 *	@file	unit_test_ranges_elements_view_sentinel_sub.cpp
 *
 *	@brief	operator- のテスト
 *
...
HAMON_CXX14_CONSTEXPR bool test00()
{
	...
	}
	{
		using Tuple = hamon::tuple<int, long>;
		using V = TestView<Tuple,
			random_access_iterator_wrapper<Tuple>,
			test_sentinel<random_access_iterator_wrapper<Tuple>>,
			random_access_iterator_wrapper<Tuple>,
			test_sentinel<random_access_iterator_wrapper<Tuple>>
		>;
		using EV = hamon::ranges::elements_view<V, 0>;
	...
	}
	{
		...
		using V = TestView<Tuple,
			random_access_iterator_wrapper<Tuple>,
			ConvertibleSentinel<random_access_iterator_wrapper<Tuple>>,
			random_access_iterator_wrapper<Tuple const>,
			ConvertibleSentinel<random_access_iterator_wrapper<Tuple const>>
		>;
		using EV = hamon::ranges::elements_view<V, 0>;
	...
	}
	{
		...
		using V = TestView<Tuple,
			...
			test_sentinel<random_access_iterator_wrapper<Tuple>>,
			random_access_iterator_wrapper<Tuple const>,
			test_sentinel<random_access_iterator_wrapper<Tuple const>>
		>;
		using EV = hamon::ranges::elements_view<V, 0>;
...
GTEST_TEST(RangesTest, ElementsViewSentinelSubTest)
{
