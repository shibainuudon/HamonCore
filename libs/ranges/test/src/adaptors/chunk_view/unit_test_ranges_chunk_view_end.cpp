/**
 *	@file	unit_test_ranges_chunk_view_end.cpp
 *
 *	@brief	end() のテスト
 *
 *	input_range:
...
HAMON_CXX14_CONSTEXPR bool test01()
{
	...
	{
		// common_range<const V> && sized_range<const V>
		{
	...
	// !simple_view<V> && forward_range<const V>
	{
		...
		// common_range<V      > && !sized_range<V      > && !bidirectional_range<V      >
		// common_range<V const> && !sized_range<V const> && !bidirectional_range<V const>
		{
			using V = TestView<int,
				forward_iterator_wrapper<int>,
				forward_iterator_wrapper<int>,
...
GTEST_TEST(RangesTest, ChunkViewEndTest)
{
