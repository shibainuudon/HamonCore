/**
 *	@file	unit_test_ranges_reverse_view_size.cpp
 *
 *	@brief	size() のテスト
 *
...
HAMON_CXX14_CONSTEXPR bool test00()
{
...

	{
		...
		static_assert(hamon::is_same<decltype(hamon::declval<RV&>().size()), int>::value, "");
		static_assert(hamon::is_same<decltype(hamon::declval<RV const&>().size()), short>::value, "");
	}
	{
		// !sized_range<V> && sized_range<V const>
		...
		static_assert(hamon::is_same<decltype(hamon::declval<RV&>().size()), unsigned int>::value, "");
		static_assert(hamon::is_same<decltype(hamon::declval<RV const&>().size()), unsigned int>::value, "");
	}
	{
		// sized_range<V> && !sized_range<V const>
		...
		static_assert(hamon::is_same<decltype(hamon::declval<RV&>().size()), char>::value, "");
	}
	{
		// !sized_range<V> && !sized_range<V const>
...
GTEST_TEST(RangesTest, ReverseViewSizeTest)
{
