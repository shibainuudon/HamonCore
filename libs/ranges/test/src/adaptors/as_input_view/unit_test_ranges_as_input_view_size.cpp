/**
 *	@file	unit_test_ranges_as_input_view_size.cpp
 *
 *	@brief	size() のテスト
 *
...
HAMON_CXX14_CONSTEXPR bool test00()
{
	//  sized_range<V> && sized_range<const V>
	{
		...
		static_assert( has_size<AIV&>::value, "");
		static_assert( has_size<AIV const&>::value, "");

		using D  = decltype(hamon::declval<AIV&>().size());
		static_assert(hamon::is_same<D, int>::value, "");

		using CD = decltype(hamon::declval<AIV const&>().size());
...
GTEST_TEST(RangesTest, AsInputViewSizeTest)
{
